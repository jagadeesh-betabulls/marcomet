...
if($height==0) {
	$height = $imageHeight;
	while($height>500) {
		$height -= (($imageWidth/$imageHeight)*$imagHeight);
	}
}
...
if (array_key_exists("x1",$_POST) && array_key_exists("y1",$_POST) && array_key_exists("x2",$_POST) && array_key_exists("y2",$_POST) && array_key_exists("width",$_POST) && array_key_exists("height",$_POST) && array_key_exists("ref",$_POST) && array_key_exists("continue",$_POST)) {
	...
	if($resURL=="") {
		$tempimg = file_get_contents(get_resource_path($ref, "",false,"jpg"));
	} else {
		$tempimg = file_get_contents($BASE_URL_HTTP_GET.$resURL);
	}
    ...
    $command.= " " . $prefix . "\"$file\"[0]".$crop_cmd." -resize $im_width"."x$im_height \"$target\""; 
...
<div id="newFileRefNotification" style="padding: 5px; border: 1px solid #CC7915; background-color: #FAB761;"><p style="display: inline;">The file has been successfully uploaded. It has been placed in the "Miscellaneous" file type category for later use.</p></div><? } ?>

<form method="post" class="form" enctype="multipart/form-data" action="">

	<div id="testWrap">
		<img src="<? if($resURL=="") { echo get_resource_path($ref, "pre",false,"jpg"); } else { echo $resURL; } ?>" alt="test image" id="testImage"  />
		<script type="text/javascript">
...
<div id="bottomInfoSubmit" style="margin: 20px 0px 0px 50px; width: 335px;">
	...
	<input type="button" name="newImage" value="Choose New Image" style="float: right;" onclick="javascript:window.location = '<? echo "/preview/template_wam/marcomet_widget.php?widget_user=$widget_user&ratioWidth=$ratioWidth&ratioHeight=$ratioHeight" . '&templateName=' . $templateName. '&imageRef=' . $imageRef. '&imageDPI=' . $imageDPI. '&imageWidth=' . $imageWidth. '&imageHeight=' . $imageHeight. '&tempImageName=' . $tempImageName. '&tempImagePath=' . $tempImagePath . "&ignoreFile=yes"; ?>';" />
