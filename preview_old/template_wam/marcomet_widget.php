...
<div style="position: absolute; left: 100px; top: 100px; z-index: 2; margin: 100px 200px 200px 200px; border: 3px solid black; width: 100px; height: 100px; background: #FFF url('/preview/template_wam/search_page_loading.gif') center no-repeat; display: none;" id="searchPageLoadingImage"></div></div>
		...
		<style type="text/css">
		.resTab {
			background: transparent url('http://wam.marcomet.com/DigitalAssets/gfx/greyblu/interface/resourcepanel.gif') repeat-x top;
		...
		.selectedTab { 
			background: transparent url('http://wam.marcomet.com/DigitalAssets/gfx/greyblu/interface/resourcepanel.gif') repeat-x bottom !important;
		...
		<div id="widgetTabBar">
			...
			<div style="top: 10px; left: 10px; position:absolute; float: left; width: 220px; height: 80px; padding: 4px; background: #E5ECEE no-repeat none; border: #779CA8 2px solid; font-size: 12px; text-align: right; 20px; display: none;" id="uploadANewFileFormBox">
				...
				<form method="post" class="form" enctype="multipart/form-data" action="http://wam.marcomet.com/DigitalAssets/marcomet_widget_create.php?widget_user=<?=$username?>&ignoreFile=<?=getval('ignoreFile', '')?>" style="display: inline; float: right;" id="uploadANewFileForm" >
		...
		function goToResChoice() {
			window.location='/preview/template_wam/marcomet_widget_nav.php?widget_user=<? echo getvalescaped("widget_user", ""); ?>&search=<?=urlencode($search)?>&order_by=<?=$order_by?>&archive=0&restypes='+document.getElementById('resourceTypeSelect').value+'&width=<?=$width?>&height=<?=$height?>&previousID=<?=$previousID?>&templateName=<?=$templateName?>&imageRef=<?=$imageRef?>&imageDPI=<?=$imageDPI?>&imageWidth=<?=$imageWidth?>&imageHeight=<?=$imageHeight?>&tempImageName=<?=$tempImageName?>&tempImagePath=<?=$tempImagePath?>&ignoreFile=<?=getval('ignoreFile', '')?>';
		}
		</script>
		<select id="resourceTypeSelect" name="resourceType" style="float: left; margin-top: 15px;" onclick="setTimeout('goToResChoice()', 1000);">
		<?
...
*/		
	...
	if ($extension2=="") {$extension2="jpg";}
			...
			$url="marcomet_view.php?brand=". $brand ."&ref=" . $ref . "&search=" . urlencode($search) . "&order_by=" . urlencode($order_by) . "&offset=" . urlencode($offset) . "&archive=" . $archive . "&restypes=" . $restypes;
...
-->
		...
		body {
			margin: 0px;
			padding: 20px;
			background: url('http://wam.marcomet.com/DigitalAssets/gfx/greyblu/interface/back.gif') repeat top;
		...
		<div class="BasicsBox"> 
		  <div class="NoFind" style="border: 1px #131B32 solid; background-color: #385D90; padding: 0px 5px 10px 5px;">
    		...
    		<a href="javascript:history.go(-1);" style="border: 1px #203B5D solid; background: transparent url('http://wam.marcomet.com/DigitalAssets/gfx/greyblu/interface/resourcepanel.gif') repeat-x; padding: 3px 3px 2px 3px; font-weight: bold; color: #203B5D; text-decoration: none;">Go Back</a>
		  </div>
		</div>
		<?
		}
