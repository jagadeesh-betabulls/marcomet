...
function GetStateFullname($state) {
	...
	if(isset($result[0])) return $result[0]["state_name"];
	else return "";
}
...
function parse_drop_filename($original_filepath, $return_res_id) {
	...
	global $tmpimport;
	if($split_file_path[0]=="Site Photos" || $tmpimport=="noodles") {
		...
		$duplicates = sql_query("SELECT * from resource_data WHERE resource_type_field = 51 AND value='".str_replace("'", "\\'", $original_filename)."' AND resource IN (SELECT ref from resource WHERE archive<>2)");
		if(isset($duplicates[0])) {
			// A duplicate exists in the database
			$version = sql_query("SELECT * from resource_data WHERE resource=".$duplicates[0]["resource"]." AND resource_type_field = 68 AND resource IN (SELECT ref from resource WHERE archive<>2)");
			if(isset($version[0])) {
				$version = $version[0]["value"];
				$version += 1;
			} else {
			...
			// Archive the old version
			$archive_old_resource = sql_query("UPDATE resource SET archive=2 WHERE ref IN (SELECT resource FROM resource_data WHERE resource_type_field=51 AND value='".str_replace("'", "\\'", $original_filename)."')");
		}
		...
		update_field($resource_ref,60,ltrim($site_number, "0"));
		update_field($resource_ref,61,ltrim($site_number, "0"));
		...
		// Get the site id
		$siteid_q = sql_query("SELECT * from resource_data WHERE resource=$resource_ref AND resource_type_field=60 OR resource=$resource_ref AND resource_type_field=61");
		if(isset($siteid_q[0])) {
			$siteid = (int)$siteid_q[0]["value"];
			
			// Get and update the city and state
			$prop = sql_query("SELECT * FROM wyndham_properties WHERE site_number=$siteid");
			$city = $prop[0]["city"];
			$state = $prop[0]["state"];
			$state = sql_query("SELECT * from lu_states WHERE state_abreviation='$state'");
			if(isset($state[0])) {
				$state = $state[0]["state_name"];
				// Update the city
				if($siteid_q[0]["resource_type_field"]=="60") update_field($resource_ref,82,$city);
				else update_field($resource_ref,80,$city);
				// Update the state
				if($siteid_q[0]["resource_type_field"]=="60") update_field($resource_ref,83,$state);
				else update_field($resource_ref,81,$state);
			}
		}
		
		// Update the Allowable Use
		$allowable_use_info = "For use by Wyndham Worldwide employees, assignees, and its brand franchise owners and their assignees.";
		update_field($resource_ref,58,$allowable_use_info);
		
		// Update the Copyright
		$copyright_info = "Wyndham Worldwide 2008";
		update_field($resource_ref,59,$copyright_info);
    ...
    // Set the property site ID
    if($resource_type==3) {
	    update_field($resource_ref,64,ltrim($site_number, "0"));
	} else {
	    update_field($resource_ref,65,ltrim($site_number, "0"));
	}
