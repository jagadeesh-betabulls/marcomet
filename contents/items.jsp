...
String brandCode = ((notAggregator || session.getAttribute("brandCode") == null || session.getAttribute("brandCode").toString().equals("")) ? "" : " AND (right(p.brand_code,1)='x' or p.brand_code='x' or p.brand_code = '" + session.getAttribute("brandCode").toString() + "') ");
...
String companyId=((request.getParameter("companyId")==null)?((session.getAttribute("tmpSHCompanyId")==null || session.getAttribute("tmpSHCompanyId").toString().equals(""))?(String)session.getAttribute("companyId"):(String)session.getAttribute("tmpSHCompanyId")):request.getParameter("companyId"));
...
String SuppressFilter=((editor)?"":" p.show_in_primary_prod_line=1 and "); 
...
String ShowRelease=((request.getParameter("ShowRelease")==null || request.getParameter("ShowRelease").equals(""))?"":" AND (p.release='"+request.getParameter("ShowRelease")+"' or p.release='') ")+brandCode;
...
String templateCategory=((request.getParameter("tempCat")==null || request.getParameter("tempCat").equals(""))?"":" AND p.template_category='"+request.getParameter("tempCat")+"' ");
boolean noTempCat=((templateCategory.equals(""))?true:false);
...
<style>
.shipPolicy {
	font-weight: bold;font-size: 9pt;font-family: Arial,Verdana,Geneva;text-decoration: none;color:red;
}
.saleLINK {
...
.pricePerItem{font-style: italic; font-weight: bold; font-size: 8pt;font-family: Arial,Verdana,Geneva;text-decoration: none;text-align: center;color: #5b6966;
}
</style>
<script>
function showInfo(){
...
<body topmargin="0" bgcolor=white id='mainBody'>
<%
...
if (useProdProps){%><form method=post><%}%><%=((showChoice)?"<br><div class=Title>STEP 2: PRODUCT CHOICE<br>Check box next to products to be included, Click Next at bottom of page.</div>":"") %><%
if (!showChoice && plCount==0){
	plCount++;
	%><div style="float: right;position: absolute;top: 0;right: 1%;"><a href="javascript:history.go(-2)" class="menuLINK">&laquo;&nbsp;Back&nbsp;</a> <a href="mailto:marketingsvcs@marcomet.com" class="menuLINK">&nbsp;Email Support&nbsp;</a></div><%
	}
...
%>
<%	if(noTempCat){
%><table width="95%" border="0" cellspacing="0" cellpadding="0" align="center" id='titleTable'><tr>
 ...
 while (rsProds.next()){
	if (x==0){
		...
		if(document.getElementById("release")!=null){
			document.getElementById("release").innerHTML='<%=((rsProds.getString("display_title")==null || rsProds.getString("display_title").equals(""))?"":rsProds.getString("display_title")+": <br>")%><%=prod_line_name%>';
		}
		</script>
		<%	if(!(noTempCat)){	
		} 
	...
	String alaQuery="Select concat('As low as $',min(round(pp.price/pp.quantity,3)),' each') as perItem from product_prices pp,products p where site_id="+siteHostId+" and pp.prod_price_code=p.prod_price_code and  p.id="+rsProds.getString("id");
	ResultSet rsPerItem = st3.executeQuery(alaQuery);
		if (rsPerItem.next() && rsPerItem.getString(1)!=null){
			...
			alaQuery="Select concat('As low as $',min(round(pp.price/pp.quantity,3)),' each') as perItem from product_prices pp,products p where site_id=0 and pp.prod_price_code=p.prod_price_code and  p.id="+rsProds.getString("id");
			rsPerItem = st3.executeQuery(alaQuery);
	...
	%><%=((noTempCat || prodNum==0)?"":"")%><tr<%=((editor)?" BGCOLOR="+statusColor:"")%> class='<%=((rsProds.getString("property_code")==null)?"":rsProds.getString("property_code"))%>' style='display:visible' >
...
if(showChoice){
	%><input type='checkbox' name='prodId_<%=prodNum%>' value='<%=rsProds.getString("id")%>' >&nbsp;<%
}
prodNum++;
...
</div><%=((rsProds.getString("ship_policy").equals("1") && rsProds.getString("show_ship_price_policy").equals("1") && noTempCat)?"<div class='shipPolicy'>NOTE: Shipping included in purchase price.</div>":"")%> <%=((rsProds.getString("ship_policy").equals("2") && rsProds.getString("show_ship_price_policy").equals("1") && noTempCat)?"<div class='shipPolicy'>NOTE: Flat Rate shipping on this product, to be applied at invoicing.</div>":"")%>
</div>
<br /><%
%>
...
</td><%if ((rsProds.getString("sale_title") !=null && !rsProds.getString("sale_title").equals("") && rsProds.getString("display_promo_w_prod").equals("1"))){
...
%><td align=center><a href="javascript:pop('/popups/promoDetails.jsp?id=<%=rsProds.getString("sale_id")%>',600,400)"  class='saleLINK'><%=rsProds.getString("sale_title")%> - Click for Details</a></td><%}else{orderText=((rsProds.getString("download_url")!=null && !rsProds.getString("download_url").equals(""))?downloadText:((!noTempCat)?"Design":"Order"));}%><td align='right'><%
...
%><%=priceList%><%=((backorderText.equals(""))?"":"<span class='subTitle' style='color:Red;'>"+backorderText+"</span>&nbsp;")%><%=((rsProds.getString("show_as_low_as_price").equals("1") && noTempCat)?perItemPrice:"")%><%
