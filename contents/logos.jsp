...
String pBrandCode=((request.getParameter("pBC")==null)?"":request.getParameter("pBC"));
...
String brandCodeStr=((session.getAttribute("brandCode")==null || session.getAttribute("brandCode").toString().equals(""))?"":(String)session.getAttribute("brandCode"));
String brandCodeGeneric=((brandCode.equals(""))?"":sl.getValue("brands","brand_code","'"+brandCode+"'", "generic"));
String bBrandCode="";
if(!pBrandCode.equals("") && pBrandCode.equals(brandCodeGeneric)){
	brandCode=((brandCode.equals(""))?"":" AND (p.brand_code='x' or p.brand_code='"+brandCodeGeneric+"') ");
	bBrandCode=((brandCode.equals(""))?"":" AND b.brand_code='"+brandCodeGeneric+"' ");
}else{
	brandCode=((brandCode.equals(""))?"":" AND (p.brand_code='x' or p.brand_code='"+brandCode+"') ");
	bBrandCode=((brandCode.equals(""))?"":" AND b.brand_code='"+brandCodeStr+"' ");
}
...
String ShowReleasePB=((request.getParameter("ShowRelease")==null || request.getParameter("ShowRelease").equals(""))?"":" AND (if(pl.pb_release_code is not null and pl.pb_release_code<>'',pl.pb_release_code=p.release,p.release='"+request.getParameter("ShowRelease")+"') ");


String prodLineId=((request.getParameter("prodLineId")==null)?"":request.getParameter("prodLineId"));

String pcompanyId=((request.getParameter("pcompanyId")==null)?((session.getAttribute("tmpSHCompanyId")!=null && !session.getAttribute("tmpSHCompanyId").toString().equals(""))?session.getAttribute("tmpSHCompanyId").toString():shs.getSiteHostCompanyId()):request.getParameter("pcompanyId"));
...
if (useProdProps && request.getParameter("rootProdCode")!=null){
	...
	while(rsPropProds.next()){
		...
		titleStr=rsPropProds.getString("title");
	}
...
if (useFullList && hProperties.size()>1){
...
<select name='defaultPropId' onChange="linkMe(this.value)"><option value="" Selected> - Select Property - </option><%
	...
	%><%=options%></select><%
}else if(hCProperties.size()==1 || (hProperties.size()==1 && titleStr.equals("ANY PROPERTY"))){
		if(hProperties.size()==1 && titleStr.equals("ANY PROPERTY")){
			Vector v = new Vector(hProperties.keySet());
	    	Collections.sort(v);
	    	Iterator e = v.iterator();
			int c=0;
...
if(prodLineId.equals("")){
	sql="SELECT distinct l.display_as_tier,if(pr.display_title is null,'',pr.display_title) display_title,p.release release_code,p.small_picurl, p.prod_code,p.id as pid, prod_name, offering_id, logo_image, brand_name FROM logos l,products p left join product_releases pr on (pr.release_code=p.release AND pr.company_id=p.company_id) where p.brand_code=l.brand_code and p.company_id=l.company_id and l.release_code=p.release and l.active=1  "+rootProdCodeFilter+variantFilter+brandCode+" and p.company_id="+pcompanyId+ShowRelease+ShowInactive+" group by prod_code ORDER BY pr.release_date desc,l.sequence";
}else{
	sql="SELECT distinct l.display_as_tier,l.sequence,if(pr.release_date is null,'',pr.release_date) release_date,pr.display_title,pr.release_code release_code,pl.id pid,logo_image, b.brand_name FROM brands b,products p,product_lines pl, logos l,product_releases pr where pl.status_id=2 and p.company_id="+pcompanyId+" AND pr.release_code=p.release AND pr.company_id=p.company_id AND ( pl.id like '"+prodLineId+"%' OR pl.top_level_prod_line_id='"+prodLineId+"') and  p.prod_line_id=pl.id and    right(pl.id,2)=b.prod_line_segment and b.company_id=p.company_id and  b.brand_code=l.brand_code and p.company_id=l.company_id and p.release=l.release_code and l.active=1  "+ShowInactive+ShowRelease+brandCode+bBrandCode+"  union SELECT distinct l.display_as_tier,l.sequence,if(pr.release_date is null,'',pr.release_date) release_date, pr.display_title,pr.release_code release_code,pl.prod_line_id pid, logo_image, b.brand_name FROM brands b,products p,product_line_bridge pl left join product_lines prl on prl.id=pl.prod_line_id, logos l,product_releases pr where (p.company_id="+pcompanyId+" or p.company_id=1066) AND pr.company_id=prl.company_id  and b.company_id=prl.company_id and l.company_id=prl.company_id AND (if(pl.pb_release_code is not null,pr.release_code=pl.pb_release_code,pr.release_code=p.release)) AND pl.status_id=2 and pl.prod_line_id like '"+prodLineId+"%'  and  p.id=pl.prod_id and right(pl.prod_line_id,2)=b.prod_line_segment and b.brand_code=l.brand_code and (if(pl.pb_release_code is not null,l.release_code=pl.pb_release_code,p.release=l.release_code)) and l.active=1  "+ShowInactive+ShowReleasePB+brandCode+bBrandCode+"    group by pid ORDER BY release_date desc,sequence";
}
...
%><%=pagemid%><%
	...
	if (y==0){
	  ...
	  if(rsProdLogos.getString("display_as_tier").equals("1")){	
%><td width=25% height='31'  valign=middle  class='logobox' onMouseOver="this.className='logoboxselected'" onMouseOut="this.className='logobox'">
