...
<jsp:useBean id="connection" class="com.marcomet.jdbc.ConnectionBean" scope="session" /><jsp:useBean id="formatter" class="com.marcomet.tools.FormaterTool" scope="page" /><jsp:useBean id="str" class="com.marcomet.tools.StringTool" scope="page" /><%
...
<body topmargin="0" bgcolor=white><%=((showChoice)?"<br><div class=Title>STEP 2: PRODUCT CHOICE<br>Check box next to products to be included, Click Next at bottom of page.</div>":"") %><%
if (!showChoice){
...
<table width="95%" cellspacing="0" cellpadding="0" align="center">
  <tr>
    <td class="body" ><%=((rsProdLine.getString("description")==null || rsProdLine.getString("description").equals(""))?"":rsProdLine.getString("description"))%></td>
...
<%if(editor){%><a href='javascript:pop("/popups/QuickChangeAddProductToLine.jsp?question=Add%20Product%20To%20Line&primaryKeyValue=<%=rsProdLine.getString("id")%>",700,150)' class=greybutton> + Add Product to this Line </a>
<a href='javascript:history.go(-1)' class=greybutton> Review Page </a>
<br><div class=subTitle>(9) ON HOLD=Show with next release (Client sees it on demo; (1) DRAFT=Hold Indefinitely (Client won't see it on demo)</div><%}%><%if(!(editor) && (((RoleResolver)session.getAttribute("roles")).roleCheck("editor"))){%><a href='javascript:location.href=location.href+"&editor=true"' class=greybutton> Edit Page </a><%}%><table border=1 cellpadding=5 cellspacing=0 width=90% align="center">
  <tr>
...
<%if(editor){
	...
	}%><tr><td ><span align='left' class="catalogLABEL"><%=((showChoice)?"<input type='checkbox' name='prodId_"+ prodNum++ +"' value='"+rsProds.getString("id")+"' >&nbsp;":"")%><%if (editor){%><a href='javascript:pop("/popups/QuickChangeForm.jsp?cols=60&rows=1&question=Change%20Product%20Name&primaryKeyValue=<%=rsProds.getString("id")%>&columnName=prod_name&tableName=products&valueType=string",500,150)'>&raquo;</a>&nbsp;<%}%><%=rsProds.getString("prod_name")%>&nbsp;-&nbsp;<%=rsProds.getString("prod_code")%></td></tr></table>
<div class="subtitle">
...
<div class="bodyBlack"><%if (editor){%><a href='javascript:pop("/popups/QuickChangeForm.jsp?cols=60&rows=6&question=Change%20Product%20Description&primaryKeyValue=<%=rsProds.getString("id")%>&columnName=detailed_description&tableName=products&valueType=string",500,200)'>&raquo;</a>&nbsp;<%}%><%=((rsProds.getString("detailed_description")!=null)?rsProds.getString("detailed_description"):"")%>
</div><br />
...
%><td align=center><a href="javascript:alert('<%=rsProds.getString("sale_desc")%>')"  class='saleLINK'><%=rsProds.getString("sale_title")%> - Click for Details</a></td><%}else{orderText=((rsProds.getString("download_url")!=null && !rsProds.getString("download_url").equals(""))?downloadText:"Order");}%><td align='right'><%
