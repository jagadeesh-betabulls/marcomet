...
<%
	...
	String brandIdPath="";
	boolean useBrandPath=false;
    Connection conn = DBConnect.getConnection();
    Statement stB = conn.createStatement();
    ResultSet rsBr = stB.executeQuery("Select * from sitehost_brandid_bridge where sitehost_id="+sitehostId);
    if (rsBr.next()) {
		useBrandPath=true;
		brandIdPath="/sitehosts/"+rsBr.getString("brandid_sitehost");
	}
    String imagePath=((useBrandPath) ? brandIdPath : (String) session.getAttribute("siteHostRoot"));
    boolean hasPic = true;
    boolean hasBigPic = false;
    boolean picBottom = ((request.getParameter("picBottom") == null) ? false : true);
    String QryStr = ((pageName.equals("")) ? "select *,concat('<img src=\"" + imagePath + "/images/',replace(small_picurl,',','\" ><br><br><img src=\"" + imagePath + "/images/'),'\">') pic  from pages where id='" + pageId + "'" : "Select *,concat('<img src=\"" + imagePath + "/images/',replace(small_picurl,',','\" ><br><br><img src=\"" + imagePath + "/images/'),'\">') pic from pages where company_id=" + shs.getSiteHostCompanyId() + " and page_name='" + pageName + "'");
...
<html>
  <head><!-- <%=QryStr%> -->
    ...
    <link rel="stylesheet" href="<%=imagePath%>/styles/vendor_styles.css" type="text/css">
  ...
  <body class="thrColLiq">
    <table width="100%" cellpadding="0" cellspacing="0" height="100%">
      <tr>
        <td valign="bottom" style="background: url(<%=imagePath%>/images/left_gradient_top.jpg) top left no-repeat;" width="50"><img src="<%=imagePath%>/images/left_gradient_bottom.jpg" align="bottom" /></td>
        <td valign="top">
          <div id="container">
            <div class=title>
              <%if (editor) {
...
<a href='javascript:pop("/popups/QuickChangeForm.jsp?cols=60&rows=1&question=Change%20Image&primaryKeyValue=<%=rs.getString("id")%>&columnName=full_picurl&tableName=pages&valueType=string",500,150)'>[Edit LARGE IMAGE POPUP URL]</a><br>
              <%}
              %>
              <%=((hasPic) ? ((hasBigPic)?"<a href='"+imagePath+"/images/"+rs.getString("full_picurl")+"' target='_blank'>":"")+rs.getString("pic")+((hasBigPic)?"</a>":""):"")%>
            </div>
...
<html>
  ...
  <body><p>This is an invalid page reference, or this page has expired: <%=pageName%>. Please click here to continue to the marketing home page: <a href="/" class='menuLINK'>HOME</a></p></body>
</html>
<%}
    }
    stB.close();
