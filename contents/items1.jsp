...
<style>
.saleLINK {
	font-weight: bold;color: white;font-size: 8pt;font-family: Arial,Verdana,Geneva;text-decoration: none;background-color: red;text-align: center;width: 8%;padding-left: 10px;padding-right: 10px;padding-top: 5px;padding-bottom: 5px;white-space: nowrap;border-top: 1px #ffffff solid;border-right: 1px #ffffff solid;border-bottom: 1px #ffffff solid;height: 27px;vertical-align: middle;
}
</style>
<script>
function showInfo(){
...
while(rsProdLine.next()){
  if (useProdProps){
	...
	while(rsContProds.next()){
		hCProperties.put(rsContProds.getString("property_id"),rsContProds.getString("property_code"));
	}
	...
	String prodpropSQL="SELECT distinct property_id,property_code,title FROM v_product_properties p  left join product_line_bridge pb on pb.prod_id=p.prod_id where company_id="+sl.getValue("site_hosts","id",siteHostId, "company_id")+" AND (("+SuppressFilter+" (p.prod_line_id="+rsProdLine.getString("id")+" or p.prod_line_id="+rsProdLine.getString("top_level_prod_line_id")+")) or pb.prod_line_id="+rsProdLine.getString("id")+") "+ShowRelease+ShowInactive+" ORDER BY property_code desc";
		%><!--contPropSQL:<%=contPropSQL%>--><!--prodpropSQL: <%=prodpropSQL%>--><%
	ResultSet rsPropProds = st.executeQuery(prodpropSQL);
	while(rsPropProds.next()){
		hProperties.put(rsPropProds.getString("property_id"),rsPropProds.getString("property_code")+"|"+rsPropProds.getString("title"));
	}
...
%><table border=1 cellpadding=5 cellspacing=0 width=90% align="center">
  <tr style='display:visible'>
	...
	%><td valign="top" class="menuLINKText" height="30">
		<div align="right" >Show Materials for Property:&nbsp;<select name='defaultPropId' onChange="document.forms[0].submit()"><%
		String options="";
		Enumeration e = hProperties.keys();
		int c=0;
		while( e.hasMoreElements() ){
			String tempProp=(String)e.nextElement();		
