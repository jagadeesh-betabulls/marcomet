...
<jsp:useBean id="sl" class="com.marcomet.tools.SimpleLookups" scope="page" /><jsp:useBean id="formatter" class="com.marcomet.tools.FormaterTool" scope="page" /><jsp:useBean id="str" class="com.marcomet.tools.StringTool" scope="page" /><%
	...
	String ShowInactive=( ( (request.getParameter("ShowInactive")==null || request.getParameter("ShowInactive").equals("")) && !(editor) )?" AND p.status_id=2 ":((request.getParameter("ShowInactive").equals("9"))?" AND p.status_id=9 ":((editor)?" AND (p.status_id<>3 and p.status_id<>4) ":" AND (p.status_id=2 OR p.status_id=9)")) ); 
	String SuppressFilter=((editor)?"":" show_in_primary_prod_line=1 and "); 
	String ShowBrand=((request.getParameter("brandCode")==null || request.getParameter("brandCode").equals(""))?"":" AND prl.brand_code= '"+request.getParameter("brandCode")+"'"); 
...
.pricePerItem {
	...
	String sql = ((prodLineId.equals(""))?"SELECT prl.* FROM product_lines prl, products p, site_hosts sh WHERE prl.status_id=2 and sh.id= " + siteHostId + " AND sh.company_id=prl.company_id AND prl.id = p.prod_line_id "+ShowRelease+ShowInactive+ShowBrand+" GROUP BY prl.id ORDER BY prl.id":"SELECT * FROM product_lines where  id="+prodLineId);
	ResultSet rsProdLine = st2.executeQuery(sql);
	int p=0;
		%></head>
<body topmargin="0" style="margin-left:0;margin-right:0;margin-top:0;margin-bottom:0;" bgcolor=white><%
  ...
  %><table border=1 cellpadding=5 cellspacing=0 width=90% align="center"><%
    ...
    <%}%>
      ...
      <span align='left' class="catalogLABEL"><%=rsProds.getString("prod_name")%>&nbsp;-&nbsp;<%=rsProds.getString("prod_code")%><%=((rsProds.getString("property_code")==null)?"":" | For Property #"+rsProds.getString("property_code"))%></span>
        ...
        <div class="bodyBlack"><%=((rsProds.getString("detailed_description")!=null)?rsProds.getString("detailed_description"):"")%></div><%=((rsProds.getString("ship_policy").equals("1"))?"<div class='shipPolicy'>NOTE: Shipping included in purchase price.</div>":"")%> <%=((rsProds.getString("ship_policy").equals("2"))?"<div class='shipPolicy'>NOTE: Flat Rate shipping on this product, to be applied at invoicing.</div>":"")%>
       </td></tr><tr>
       <td valign="top" ><%=((rsProds.getString("small_picurl")==null || rsProds.getString("small_picurl").equals(""))?"":"<img  width=155 src='"+siteHostRoot+"/fileuploads/product_images/"+ str.replaceSubstring(rsProds.getString("small_picurl")," ","%20") +"' border=0>")%> </td>
        ...
        int y=0;
		ResultSet rsPrices = st3.executeQuery("Select format(quantity,0) as quantity,format(price,2) as price,format(price/quantity,3) as perItem  from product_prices pp,products p where pp.prod_price_code=p.prod_price_code and quantity>0 and price >.009 and p.id="+rsProds.getString("id")+" ORDER BY pp.quantity");
		while (rsPrices.next()){
