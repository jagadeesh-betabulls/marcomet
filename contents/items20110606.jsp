 ...
 while (rsProds.next()){
	...
	ResultSet rsPerItem = st3.executeQuery(alaQuery);
		if (rsPerItem.next() && rsPerItem.getString(1)!=null){
			...
			alaQuery="Select concat('As low as $',min(round(pp.price/pp.quantity,3)),' each') as perItem from product_prices pp,products p where pp.prod_price_code=p.prod_price_code and  p.id="+rsProds.getString("id");
...
</td><%if ((rsProds.getString("sale_title") !=null && !rsProds.getString("sale_title").equals("") && rsProds.getString("show_prod_promo").equals("1"))){
