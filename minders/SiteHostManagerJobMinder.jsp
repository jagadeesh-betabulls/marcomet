<%@ page errorPage="/errors/ExceptionPage.jsp" %><%@ page import="java.sql.*,java.util.*,com.marcomet.tools.*,com.marcomet.environment.*;
...
if (request.getParameter("actionFilter") != null) {
   ...
   session.setAttribute("actionFilter", " AND 1 = 1 ");
	 ...
	 %><html>
<head>
  <title>Site Host Manager Job Minder</title>
...
<span id='loading'><div align='center'> L O A D I N G . . .<br><br><img src='/images/loading.gif'><br></div></span><div id='filtertoggle' ></div><div id='filters'><%@ include file='/includes/shmminderfilters.jsp' %></div><%
...
//if the action filter is set to show jobs with files that need to be submitted need to run query twice and exclude the jobs that have files submitted.
	...
	if (jobnumberFilter.equals("") && !(actionFilter.equals(""))){
		 ...
		 fmd2Add1=((actionFilter.indexOf("fmd2")>0)?",file_meta_data fmd2 ":"");
		 fmd2Add2=((actionFilter.indexOf("fmd2")>0)?" fmd2.job_id=j.id AND ":"");		
	}
	String baseSQL = "SELECT o.id 'order_id', DATE_FORMAT(o.date_created,'%m/%d/%y') 'creation_date', j.id 'job_id', j.job_name 'job_name', ljt.value 'job_type', lst.value 'service_type', j.status_id 'status_id', j.billable 'invoice', j.billed, j.collected 'collected', j.escrow_amount 'escrow', jfs.shownstatus 'shown_status', CONCAT(c.firstname,' ',c.lastname) 'customer', j.balance_to_vendor 'vendorbalance', j.balance_to_site_host 'sitehostbalance', consolereseller, consolecustomer, actionform, whosaction, sh.abbreviation 'sitehost',c2.company_name 'sitehostname', p.id 'projectnumber', c.default_site_number sitenumber, c.default_pm_site_number pmsitenumber, c.id 'customerid', o.site_host_contact_id 'sitehostcontactid', j.vendor_company_id 'vendor_company_id', j.vendor_notes 'internalref', j.subvendor_reference_data 'subvendorref', j.vendor_contact_id 'vendorcontactid', c1.company_name 'vendor', j.buyer_internal_reference_data, j.job_link, o.buyer_contact_id, o.buyer_company_id, c3.company_name 'buyercompany', j.root_prod_code, j.quantity 'quantity',j.dropship_vendor 'dropship_vendor' FROM orders o, projects p, contacts c, jobflowstates jfs, lu_job_types ljt, lu_service_types lst, vendors v, companies c1, companies c2, companies c3, site_hosts sh, job_specs js, jobs j "+fmd2Add1+" ";
...
minderQuery.append(" LEFT JOIN products prod ON j.product_id = prod.id ");
minderFilterQuery.append(" LEFT JOIN products prod ON j.product_id = prod.id ");
...
if (!jobnumberFilter.equals("")){
	...
	filterSQL.append(archiveFilter);
	filterSQL.append(collectedFilter);
	filterSQL.append(companyFilter);
	filterSQL.append(sitenumberFilter);
	filterSQL.append(siteHostFilter);
	filterSQL.append(vendorRepFilter);
	filterSQL.append(vendorFilter);
	filterSQL.append(rootProdFilter);
	filterSQL.append(designFilter);	
	filterSQL.append(jobTypeFilter);
	filterSQL.append(subvendorFilter);
...
if (minderRS.getString("internalref")==null || minderRS.getString("internalref").equals("")){
	...
	jobNotes=minderRS.getString("internalref").substring(0,((minderRS.getString("internalref").indexOf("</div>")==-1)?100:minderRS.getString("internalref").indexOf("</div>")));
}
...
//vendor notes
%><td class="lineitemleft<%=(alt)?"alt":""%>"><%=jobNotes%>&nbsp;</td><%
...
// job amounts
	%><td class="lineitemright<%=(alt)?"alt\"":"\""%>><%=formatter.getCurrency(minderRS.getDouble("invoice"))%><br><%=formatter.getCurrency(minderRS.getDouble("billed"))%><br><%=formatter.getCurrency(minderRS.getDouble("collected"))%></td><%-- subcontracting amounts --%>
<td class="lineitemcenter<%=(alt)?"alt\"":"\""%> nowrap>&nbsp;&nbsp;
<%=((minderRS.getString("dropship_vendor")==null || minderRS.getString("dropship_vendor").equals("") || minderRS.getString("dropship_vendor").equals("0"))?"":"<a class='minderLink' href=\"javascript:pop('/popups/PurchaseOrder.jsp?jobId="+minderRS.getString("job_id")+"&actingRole=vendor','900','700')\" class='minderLink' onmouseout=\"popUpTT(event,'po')\" onmouseover=\"popUpTT(event,'po')\"><b>Print&nbsp;PO:<br></b>SubCon#"+minderRS.getString("dropship_vendor")+"</a>"+((minderRS.getString("subvendorref").equals(""))?"":"<br>Ref&nbsp;#&nbsp;"+minderRS.getString("subvendorref")))%></td><%
...
String invoiceQuery = "SELECT ari.id 'id', ari.invoice_number, ari.creation_date, ari.ar_invoice_amount, sum(arcd.payment_amount) as invoice_payments, ari.ar_invoice_amount - (if(sum(arcd.payment_amount) is null,0,sum(arcd.payment_amount))) as invoice_balance FROM ar_invoices ari, ar_invoice_details arid LEFT JOIN ar_collection_details arcd on arcd.ar_invoiceid=ari.id WHERE ari.id = arid.ar_invoiceid AND arid.jobid = " + minderRS.getString("job_id") + " GROUP BY ari.invoice_number having (invoice_balance) >.01 or (invoice_balance) < -.01 ORDER BY ari.creation_date";
   ResultSet invoiceRS = st.executeQuery(invoiceQuery);
   while (invoiceRS.next()) { 
	%><a class="minderACTION" href="/minders/workflowforms/ReviewInvoice.jsp?invoiceId=<%=invoiceRS.getString("id")%>&jobId=<$ job_id $>"><%=(makeBreak ? "<br>":"")%>Pay Invoice #<%=invoiceRS.getString("ari.invoice_number")+"&nbsp;/&nbsp;"+formatter.getCurrency(invoiceRS.getString("invoice_balance"))+ ((invoiceRS.getDouble("invoice_balance")>0)?"&nbsp;Due ":"&nbsp;Credit ")%></a><% makeBreak = true;
 } } 
%>&nbsp;</td><%
...
makeBreak = true;
   ...
   ResultSet fileRS = st.executeQuery(fileQuery);
   while (fileRS.next()) { 
...
//Other actions
%><td class="lineitemleft<%=((alt)?"alt":"")%>" nowrap>&nbsp;&nbsp;<a href="javascript:pop('/popups/JobDetailsPage.jsp?jobId=<$ job_id $><%=jobRole%>','700','600')" class="minderLink">Details</a> | <a href="/files/JobFileViewer.jsp?jobId=<$ job_id $>" class="minderLink">Files</a><a class="minderLink" href="/files/useremailform.jsp?jobId=<$ job_id $>&target=client"><br>&nbsp;&nbsp;Email Client</a><a class="minderLink" href="/files/useremailform.jsp?jobId=<$ job_id $>&target=vendor"><br>&nbsp;&nbsp;Email Rep</a></td><%
} %></tr><%
i++;
}
...
<script>
	...
	function togglefilters(){
		...
		if (style2.display=="block"){
			document.getElementById('filtertoggle').innerHTML='<div><a href="javascript:togglefilters()" class=menuLINK >+ Show Filters</a></div>';
		}else{
			document.getElementById('filtertoggle').innerHTML=<%=((i==0)?"''":"'<div><a href=\"javascript:togglefilters()\" class=menuLINK>&raquo; Hide Filters</a></div>'")%>;
		}
		toggleLayer('filters');
	}
	<%=((i==0)?"togglefilters()":"document.getElementById('filtertoggle').innerHTML='<div><a href=\"javascript:togglefilters()\" class=menuLINK>+ Show Filters</a></div>'")%>
</script>
</body>
</html><%
try{
