...
<td>&nbsp;</td>
  ...
  if (changes) {
    ...
    sql = "SELECT l.state 'stateid', co.tax_exempt 'exempt'  FROM locations l, jobs j, projects p, orders o,companies co,contacts c WHERE o.id = p.order_id AND p.id = j.project_id AND l.contactid=o.buyer_contact_id  AND c.id=o.buyer_contact_id and c.companyid=co.id AND l.locationtypeid=1 AND j.id = " + jobId;
	 ResultSet rsTaxInfo = st.executeQuery(sql);
	if (rsTaxInfo.next()) {
		taxableState=rsTaxInfo.getString("stateid");
		buyerExempt=rsTaxInfo.getString("exempt");
	}
	if (taxableState.equals("31")){
		taxState="31";
		taxRate="7";
		taxShipping="1";
		taxJob="1";
...
<!--    <tr> 
  ...
  <table border="0" align="center">
    <tr>
      <td width="3%">&nbsp;</td>
      <td><a href='javascript:<%=((hideAction)?"none();":"proposeChange()")%>' id="propButton" class="greybutton">Propose Change</a></td>
      <td width="3%"></td><%
