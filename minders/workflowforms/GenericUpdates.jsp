...
String primarySQL="SELECT * from jobs "+((jobId.equals("0"))?"":" WHERE id="+jobId);
...
while(rsRecords.next()){
		...
		String updatePrimary="update jobs j set j.Accrued_PO_Bal = "+accruedPO+", j.Accrued_Ship_Bal = "+accruedShipBal+", j.Last_Accrual_Refresh_Date = Now()";	
			st3.execute(updatePrimary);
			%><tr><td><%=jobId%></td><td><%=accruedPO%></td><td><%=accruedShipBal%></td></tr><%
		}
	st.close();
	st2.close();
	st3.close();
