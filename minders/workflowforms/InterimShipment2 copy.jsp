...
String query = "";
query = "SELECT j.post_date,j.ship_cost_policy,j.ship_price_policy,j.std_ship_cost,j.std_ship_price,concat(b.city,', ',blu.value,' ', b.zip) buyer_address,concat(v.city,', ',vlu.value,' ', v.zip) vendor_address, jbuyer_contact_id, j.dropship_vendor,j.vendor_contact_id, sales_contact_id site_host_contact_id, site_host_global_markup, marcomet_global_fee, if(quantity is null,'0',quantity) quantity, if(quantity_shipped is null,'0',quantity_shipped) quantity_shipped,j.product_id,j.price FROM lu_abreviated_states blu, lu_abreviated_states vlu, locations b, company_locations v,vendors vn, jobs j WHERE vn.id='"+proxyVendorId+"' and (b.contactid=j.jbuyer_contact_id and b.locationtypeid=1)  and (v.company_id = vn.company_id and v.lu_location_type_id=4) and vlu.id=v.state and blu.id=b.state AND j.id = " + request.getParameter("jobId");
...
query="select p.inventory_initialized,p.inventory_amount,p.id,if(p.root_inv_prod_id=0,p.id,p.root_inv_prod_id) inv_id, if(p.root_inv_prod_id=0,p.prod_code,ip.prod_code) inv_prod_code,if(pc.id is null,0,pc.std_ship) shipping, if(pc.id is null,sh.default_mc_handling,pc.std_vend_hand) 'vendor_handling',format(if(pc.id is null,0,pc.std_subvendor_hand),2) 'subvendor_handling', if(pc.max_percent_shipprice=0.00,100,pc.max_percent_shipprice) as 'max_percent_shipprice', if(pc.max_percent_jobprice=0.00,100,pc.max_percent_jobprice) as 'max_percent_jobprice', (j.quantity-(if(sd.shipping_quantity is null,0,sd.shipping_quantity))) balance, concat(v.city,', ',vlu.value,' ', v.zip) ds_vendor_address, p.prod_code, p.prod_name, if(p.default_warehouse_id=0,j.jwarehouse_id,p.default_warehouse_id) 'default_warehouse_id', p.inventory_product_flag from site_hosts sh,products p left join product_prices ppc on ppc.prod_price_code=p.prod_price_code left join vendors vn on vn.id='"+proxyVendorId+"'  left join products ip on p.root_inv_prod_id=ip.id left join company_locations v  on v.company_id=vn.company_id left join lu_abreviated_states vlu on  vlu.id=v.state, jobs j left join shipping_data sd on sd.job_id=j.id left join product_prices pc on j.quantity=pc.quantity where j.jsite_host_id=sh.id and p.id = j.product_id  and p.prod_price_code=pc.prod_price_code AND j.id ="+request.getParameter("jobId")+" group by j.id";


//query = "select p.inventory_initialized,p.inventory_amount,p.id,if(p.root_inv_prod_id=0,p.id,p.root_inv_prod_id) inv_id, if(p.root_inv_prod_id=0,p.prod_code,ip.prod_code) inv_prod_code,if(pc.id is null,0,pc.std_ship) shipping, if(pc.id is null,sh.default_mc_handling,pc.std_vend_hand) 'vendor_handling',format(if(pc.id is null,0,pc.std_subvendor_hand),2) 'subvendor_handling', (j.quantity-(if(sd.shipping_quantity is null,0,sd.shipping_quantity))) balance, concat(v.city,', ',vlu.value,' ', v.zip) ds_vendor_address, p.prod_code, p.prod_name,if(p.default_warehouse_id=0,v.id,p.default_warehouse_id) 'default_warehouse_id', p.inventory_product_flag from site_hosts sh,products p left join product_prices ppc on ppc.prod_price_code=p.prod_price_code left join vendors vn on vn.id='"+proxyVendorId+"'  left join products ip on p.root_inv_prod_id=ip.id left join company_locations v  on v.company_id=vn.company_id left join lu_abreviated_states vlu on  vlu.id=v.state, jobs j left join shipping_data sd on sd.job_id=j.id left join product_prices pc on j.quantity=pc.quantity where j.jsite_host_id=sh.id and p.id = j.product_id  and p.prod_price_code=pc.prod_price_code AND j.id ="+request.getParameter("jobId")+" group by j.id";
...
if (!((String)rehash.get("product_id")).equals("0")){
	if(continueFl){ //only check for product warehouses if the inventory was initialized
		query="select  '1' as type, p.id 'product_id', concat(v.city,', ',vlu.value,' ', v.zip) vendor_address,i.warehouse_id warehouse_id, sum(i.amount) balance, concat(c.company_name,',',v.address1,',',v.city,', ',vlu.value,' ', v.zip,': ',sum(i.amount)) warehouse, v.id  from inventory i, jobs j,lu_abreviated_states vlu, companies c, company_locations v ,products p where v.id=i.warehouse_id and c.id=v.company_id  and vlu.id=v.state  and i.root_inv_prod_id =if(p.root_inv_prod_id=0,p.id, p.root_inv_prod_id)  and p.id=j.product_id AND j.id ="+request.getParameter("jobId")+" group by j.product_id union ";
	}
query+="select '2' as type, p.id 'product_id', concat(v.city,', ',vlu.value,' ', v.zip) vendor_address,v.id warehouse_id, 0 as balance, concat(c.company_name,',',v.address1,',',v.city,', ',vlu.value,' ', v.zip,': N/A') warehouse, v.id  from jobs j,lu_abreviated_states vlu, companies c, company_locations v , products p, vendors vn where p.id = j.product_id and (vn.id=j.dropship_vendor) and c.id=vn.company_id  and v.company_id=c.id and vlu.id=v.state  AND j.id ="+request.getParameter("jobId")+"  and v.lu_location_type_id=4 union select '3' as type, p.id 'product_id', concat(v.city,', ',vlu.value,' ', v.zip) vendor_address,v.id warehouse_id, 0 as balance, concat(c.company_name,',',v.address1,',',v.city,', ',vlu.value,' ', v.zip,': N/A') warehouse, v.id  from jobs j,lu_abreviated_states vlu, companies c, company_locations v , products p, product_price_codes ppc,vendors vn where p.id = j.product_id and p.prod_price_code=ppc.prod_price_code and (vn.id=j.vendor_id) and c.id=vn.company_id  and v.company_id=c.id and vlu.id=v.state AND j.id ="+request.getParameter("jobId")+" and v.lu_location_type_id=4 ";

query+=" union select '3' as type, p.id 'product_id', concat(v.city,', ',vlu.value,' ', v.zip) vendor_address,v.id warehouse_id, 0 as balance, concat(c.company_name,',',v.address1,',',v.city,', ',vlu.value,' ', v.zip,': N/A') warehouse, v.id  from jobs j,lu_abreviated_states vlu, companies c, company_locations v , products p, vendors vn where p.id = j.product_id and (vn.id=j.dropship_vendor) and c.id=vn.company_id  and v.company_id=c.id and vlu.id=v.state  AND j.id ="+request.getParameter("jobId")+"  and v.lu_location_type_id=4 union select '3' as type, p.id 'product_id', concat(v.city,', ',vlu.value,' ', v.zip) vendor_address,v.id warehouse_id, 0 as balance, concat(c.company_name,',',v.address1,',',v.city,', ',vlu.value,' ', v.zip,': N/A') warehouse, v.id  from jobs j,lu_abreviated_states vlu, companies c, company_locations v , products p, product_price_codes ppc,vendors vn where p.id = j.product_id and p.prod_price_code=ppc.prod_price_code and (vn.id='"+proxyVendorId+"') and c.id=vn.company_id  and v.company_id=c.id and vlu.id=v.state AND j.id ="+request.getParameter("jobId")+" and v.lu_location_type_id=4 group by vendor_address order by type ";
...
String sltStr="";
while (rs.next()) {
  if (wareHash.get(rs.getString("warehouse_id"))==null){
  ...
  <tr>
    ...
    <table border="1" cellpadding="5" cellspacing="0" >
      <tr>
        ...
        <%=((rehash.get("shipPricePolicy").toString().equals("1"))?"Note: Shipping Price Included<br>":"")%>
        <%=((rehash.get("shipPricePolicy").toString().equals("2"))?"Note: Standard Shipping Price of $"+rehash.get("stdShipPrice").toString()+" Applies<br>":"")%></span>
