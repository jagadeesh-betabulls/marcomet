...
// CSA added
    ...
    String jobinprogressquery = "";
	jobinprogressquery = "SELECT status_id as job_status FROM jobs WHERE id = " + request.getParameter("jobId");

    ResultSet rsjobinprogress = st1.executeQuery(jobinprogressquery);
	
	if(rsjobinprogress.next()) { } else {
		response.sendRedirect("/popups/QuickShipForm.jsp");
        return;
		}
	
    if(!rsjobinprogress.getString("job_status").equals("112")) { 
		response.sendRedirect("/popups/QuickShipForm.jsp");
        return;
  		}  
...
<%if (shipAdjust) {%>
  ...
  function PackingSlip() {
    var method = document.getElementsByName("method").item(0).value;
    var reference = document.getElementsByName("reference").item(0).value;
    var shipDate = document.getElementsByName("adjdate").item(0).value;
    var url = "/minders/workflowforms/PackingSlip.jsp?jobId="+<%=request.getParameter("jobId")%>+"&carrier=UPS&method="+method+"&reference="+reference+"&shipDate="+shipDate;
  ...
  <body class="body" onLoad="MM_preloadImages('/images/buttons/submitover.gif','/images/buttons/cancelbtover.gif');document.forms[0].shippedTo.focus();" background="<%=((SiteHostSettings) session.getAttribute("siteHostSettings")).getSiteHostRoot()%>/images/back3.jpg">
...
<!-- Start CSA Add -->
    ...
    String priorshipquery = "";
	priorshipquery = "SELECT job_id FROM shipping_data WHERE job_id = " + request.getParameter("jobId");

    ResultSet rspriorship = st1.executeQuery(priorshipquery);
    while (rspriorship.next()) { %>
			  <p class="Title"><font color="#FF0000" size=5><b>**Data Entry Alert -- A Shipment Has Alerady Been Entered On This Job**</b></font></p>
...
<!-- End CSA Add -->
    ...
    <table border="1" cellpadding="5" cellspacing="0" >
        ...
        <tr>
          ...
          <td class="label" valign='top'><input type="hidden" name="adjdate" id="f_adjdate_d" onFocus="this.select()"><span class="lineitemsright" id="show_d">Choose Ship Date</span>&nbsp;<img src="/javascripts/jscalendar/img.gif" align="absmiddle" id="f_trigger_c"
...
<td> <div align="center"><a href="javascript:SubmitForm('<%=completeAction%>')"class="greybutton">Process Shipment &amp; Mark Job Complete</a></div></td>
                <td> <div align="center"><a href="javascript:SubmitForm('<%=openAction%>')"class="greybutton" tabindex="-1">Process Shipment &amp; Leave Job Open</a></div></td>
                <td> <div align="center"><a disabled href="#" class="greybutton" tabindex="-1">Process Shipment &amp; Print Packing Slip</a></div></td>
                <td> <div align="center"><a href="javascript:PackingSlip()" class="greybutton" tabindex="-1">Print Packing Slip</a></div></td>
                <td>
                  <div align="center"><a href="<%=((closeOnExit) ? "javascript:window.close();" : "/popups/QuickShipForm.jsp")%>"class="greybutton">Cancel</a></div>
                </td>
      <%} else {%><td> <div align="center"><a href="javascript:SubmitForm('102')"class="greybutton">Apply Adjustment</a></div></td>                <td>
                  <div align="center"><a href="<%=((closeOnExit) ? "javascript:window.close();" : "/popups/QuickShipForm.jsp")%>"class="greybutton">Cancel</a></div>
      ...
      <input type="hidden" name="$$Return" value="<%=((!(closeOnExit)) ? "[/popups/QuickShipForm.jsp]" : "<script>" + ((request.getParameter("refreshOpener") != null) ? "" : "parent.window.opener.location.reload();") + " setTimeout('close()',500);</script>")%>">
