...
String query = "update tmp_shipping_data t, shipping_data s set s.reference=t.reference,s.method=t.method  where s.job_id=t.job_id and s.cost=t.cost and s.reference<>t.reference and s.reference='see warehouse'";
...
query="Select t.*,j.product_id,j.quantity,if(p.root_inv_prod_id=0,p.id,p.root_inv_prod_id) root_inv,p.prod_code from tmp_shipping_data t,jobs j,products p left join shipping_data s on s.job_id=t.job_id and s.reference=t.reference where t.job_id=j.id and s.id is null and j.product_id=p.id";
int x=0;
