...
query="Select t.*,j.product_id,j.quantity,if(p.root_inv_prod_id=0,p.id,p.root_inv_prod_id) root_inv,p.prod_code from tmp_shipping_data t,products p,jobs j left join shipping_data s on s.job_id=t.job_id and s.cost=t.cost where t.job_id=j.id and s.id is null and j.product_id=p.id";
int x=0;
...
while (rs.next()) {
	...
	st2.execute(query);
	String shipSQL="select max(id) from shipping_data where job_id='"+rs.getString("t.job_id")+"' and date='"+rs.getString("t.date")+"'";
	String shipId="";
	ResultSet rsSh = st2.executeQuery(shipSQL);
	if (rsSh.next()){
		shipId=rsSh.getString(1);
	}
	query = "insert into inventory set job_id='"+rs.getString("t.job_id")+"',ship_id='"+shipId+"', adjustment_notes='PLI Shipment, auto-imported for DI Keycard Holders',root_inv_prod_code='"+rs.getString("p.prod_code")+"',adjustment_type_id='1',adjustment_action='0',product_id='"+rs.getString("j.product_id") +"',root_inv_prod_id='"+rs.getString("root_inv")+"',amount=-"+rs.getString("j.quantity")+",adjustor_contact_id ='"+rs.getString("t.user_id")+"',adjustment_date ='"+rs.getString("t.date")+"',warehouse_id ='"+rs.getString("t.warehouse_id")+"'";
	st2.execute(query);
	...
	iu.calculate(rs.getString("t.job_id"));
