...
String svAssignedFilter = "";
String svReferenceFilter = "";
String svReference = "";
...
if (request.getParameter("svAssignedFilter") != null) {
   svAssignedFilter = request.getParameter("svAssignedFilter");
   session.setAttribute("svAssignedFilter", request.getParameter("svAssignedFilter"));
} else if (session.getAttribute("svAssignedFilter") != null) {
   svAssignedFilter = (String)session.getAttribute("svAssignedFilter");
} else {
   svAssignedFilter = " AND 1 = 1 ";
   session.setAttribute("svAssignedFilter", " AND 1 = 1 ");
}
...
if (sitenumberEntry.equals("")) {
	   sitenumberFilter = " AND 1 = 1 ";
   		...
   		sitenumberFilter = " AND j.site_number = '"+sitenumberEntry+"' ";
}

if (request.getParameter("customerIDEntry") != null) {
   customerIDEntry = request.getParameter("customerIDEntry");
   session.setAttribute("customerIDEntry", request.getParameter("customerIDEntry"));
} else if (session.getAttribute("customerIDEntry") != null) {
   customerIDEntry = (String)session.getAttribute("customerIDEntry");
} else {
   customerIDEntry = "";
   session.setAttribute("customerIDEntry", "");
}

if (customerIDEntry.equals("")) {
	   customerIDFilter = " AND 1 = 1 ";
	}else{
   		customerIDFilter = " AND j.jbuyer_contact_id = '"+customerIDEntry+"' ";
}
...
if (request.getParameter("dateFilterStr") != null) {
   dateFilterStr = request.getParameter("dateFilterStr");
   session.setAttribute("dateFilterStr", request.getParameter("dateFilterStr"));
} else if (session.getAttribute("dateFilterStr") != null) {
   dateFilterStr = (String)session.getAttribute("dateFilterStr");
} else {
   dateFilterStr = " AND 1 = 1 ";
   session.setAttribute("dateFilterStr", " AND 1 = 1 ");
}
...
dateFilter=dateFilterStr.replace("dateBegin","'"+dateFrom+"'").replace("dateEnd","'"+dateTo+" 23:59:59'");
dateFilter.equals((dateFrom.equals("") || dateTo.equals(""))?" ":dateFilter);
...
//System.out.println("mndrJobNumber is "+mndrJobNumber);
svReference=((request.getParameter("svReference")== null)?((session.getAttribute("svReference") == null)?"":session.getAttribute("svReference").toString()):request.getParameter("svReference"));
session.setAttribute("svReference", svReference);
...
orderFilter=request.getParameter("orderNumber");
}else{
	orderFilter="";
}
svReferenceFilter = ((svReference.equals(""))?"":" AND j.subvendor_reference_data = '"+ svReference + "' ");
...
if (request.getParameter("actionFilter") != null) {
   actionFilterStr = request.getParameter("actionFilter");
   session.setAttribute("actionFilter", request.getParameter("actionFilter"));
} else if (session.getAttribute("actionFilter") != null) {
   actionFilterStr = (String)session.getAttribute("actionFilter");
} else {
   actionFilter = " AND 1 = 1 ";
   session.setAttribute("actionFilter", " AND 1 = 1 ");
}

if (actionFilterStr.equals("1")){
	actionFilter=" AND fmd.id is null ";
	fmd2Query=" AND (((fmd.category='For Appvl' or fmd.category='Final') and (fmd.status='Final' or fmd.status='Submitted' or fmd.Status='Accepted')))  ";
}else if (actionFilterStr.equals("2")){
	actionFilter=" AND fmd.status='Submitted' AND fmd.category='For Appvl' ";
}else if (actionFilterStr.equals("3")){
	actionFilter=" AND fmd.status='Submitted' AND fmd.category='For Appvl' AND ((TO_DAYS(NOW())-TO_DAYS(fmd.creation_date)) > 3) ";
}else if (actionFilterStr.equals("4")){
	actionFilter=" AND fmd.status='Accepted' AND fmd.category='For Appvl' ";
}else if (actionFilterStr.equals("5")){
	actionFilter=" AND fmd2.status='Accepted' and fmd2.category='For Appvl' and fmd.id is null ";
}else if (actionFilterStr.equals("6")){
	actionFilter=" AND fmd.status='Final' ";
}
...
<span id='loading'><div align='center'> L O A D I N G . . .<br><br><img src='/images/loading.gif'><br></div></span><div id='filtertoggle' ></div><div id='filters'><%@ include file='/includes/shvminderfilters.jsp' %></div>
<script>
...
//if the action filter is set to show jobs with files that need to be submitted need to run query twice and exclude the jobs that have files submitted.
	...
	if (jobnumberFilter.equals("") && svReferenceFilter.equals("") && !(actionFilter.equals(""))){
...
minderQuery.append(" LEFT JOIN vendors sv on j.dropship_vendor=sv.id LEFT JOIN products prod ON j.product_id = prod.id LEFT JOIN v_ship_to_location shipto on j.ship_location_id=shipto.id ");

minderFilterQuery.append(" LEFT JOIN vendors sv on j.dropship_vendor=sv.id LEFT JOIN products prod ON j.product_id = prod.id LEFT JOIN v_ship_to_location shipto on j.ship_location_id=shipto.id ");
...
}else if(!svReferenceFilter.equals("")){
	minderQuery.append(svReferenceFilter);
	minderQuery.append(" GROUP BY j.id");
}else if(request.getParameter("orderNumber")!=""){
	minderQuery.append(" AND (o.id = "+ orderNumber +")");
	minderQuery.append(" GROUP BY j.id");
}
...
else
{
	...
	filterSQL.append(dateFilter);
	filterSQL.append(((archiveFilter.equals(" AND 1 = 1 "))?"":archiveFilter));
	filterSQL.append(((svAssignedFilter.equals(" AND 1 = 1 "))?"":svAssignedFilter));
	...
	filterSQL.append(((customerIDFilter.equals(" AND 1 = 1 "))?"":customerIDFilter));
...
System.out.println("minderQuery is "+minderQuery);
...
%><td class="lineitemleft<%=(alt)?"alt":""%>"><a href="javascript:pop('/popups/PersonProfilePage.jsp?personId=<$ customerid $>','650','450')" class="minderLink" onmouseout="popUpTT(event,'cdets')" onmouseover="popUpTT(event,'cdets')">Cust #:&nbsp<%=minderRS.getString("customerid") %><%=((minderRS.getString("sitenumber")==null || minderRS.getString("sitenumber").equals("") || minderRS.getString("sitenumber").equals("0"))?"":", Site #:&nbsp;"+minderRS.getString("sitenumber"))%><br><$ buyercompany $><br><$ customer $></a><%=((minderRS.getString("buyer_internal_reference_data") == null || minderRS.getString("buyer_internal_reference_data").equals(""))?"":"<br>Buyer&nbsp;Ref:&nbsp;"+minderRS.getString("buyer_internal_reference_data") )%><br><%=minderRS.getString("st_address1")%><%=((minderRS.getString("st_address2") == null || minderRS.getString("st_address2").equals(""))?"":"<br>"+minderRS.getString("st_address2") )%><br><%=minderRS.getString("st_city")%>,&nbsp<%=minderRS.getString("st_state")%>&nbsp<%=minderRS.getString("st_zip")%><br><%=minderRS.getString("st_country")%></td><%
...
<td class="lineitemcenter<%=(alt)?"alt":""%>"><%=minderRS.getString("order_id")%><br><br><%=minderRS.getString("creation_date")%></td><%
...
String invoiceQuery = "SELECT ari.id 'id', ari.invoice_number, ari.creation_date, ari.ar_invoice_amount, sum(arcd.payment_amount) as invoice_payments, ari.ar_invoice_amount - (if(sum(arcd.payment_amount) is null,0,sum(arcd.payment_amount))) as invoice_balance FROM ar_invoice_details arid,ar_invoices ari LEFT JOIN ar_collection_details arcd on arcd.ar_invoiceid=ari.id WHERE ari.id = arid.ar_invoiceid AND arid.jobid = " + minderRS.getString("job_id") + " GROUP BY ari.invoice_number having (invoice_balance) >0 ORDER BY ari.creation_date";
   ResultSet invoiceRS = st2.executeQuery(invoiceQuery);
   if (invoiceRS.next()) { 
	%><a class="minderACTION" href="/minders/workflowforms/ReviewInvoice.jsp?invoiceId=<%=invoiceRS.getString("id")%>&jobId=<$ job_id $>"><%=(makeBreak ? "<br>":"")%>Invoice&nbsp;#<%=invoiceRS.getString("ari.invoice_number")+"&nbsp;/ "+formatter.getCurrency(invoiceRS.getString("invoice_balance"))+ ((invoiceRS.getDouble("invoice_balance")>0)?"&nbsp;Due ":"&nbsp;Credit ")%></a><%
	 makeBreak = true;
 } 
} 
%>&nbsp;</td><%
