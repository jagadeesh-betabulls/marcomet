...
<% boolean alt = true;
   ...
   if(minderRS.next()){
	   ...
	   quantity=minderRS.getString("quantity");
	   creationDate=minderRS.getString("creation_date");
   }
  // System.out.println(minderRS);
   %>
...
<!--    <td class="lineitemcenter<%=(alt)?"alt\"":"\""%><$ order_id $></td> -->
    <%-- order date --%>
    <td class="lineitemcenter<%=(alt)?"alt\"":"\""%><%=creationDate%></td>
    <%-- project number --%>
<!--    <td class="lineitemcenter<%=(alt)?"alt\"":"\""%><$ projectnumber $></td> -->
	...
	<%=(quantity!=null&&quantity!="")?"Qty: " +(quantity):("")%>
	</td>
<!-- Suppress Job Type and Service Type
    <%-- service type --%>
