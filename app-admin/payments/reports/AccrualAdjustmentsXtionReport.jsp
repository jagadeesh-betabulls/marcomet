...
String reportTitle="Job Accrual Adjustments Transaction Report (Adj to Original PO)";
...
if(stt.equals("v")){
	adjSQL="Select j.id 'K:H:',poa.vendor_id 'S:Vendor #',poa.id 'Po:PO Adjustment #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(poa.adjustment_date,'%m/%d/%Y') 'Adjustment Date',DATE_FORMAT(poa.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',poa.job_id 'K:Job #',j.job_name 'Job Name',poa.adjustment_code 'Adj Code',poa.adjustment_amount 'T:R:Accrued Job Cost Adj',poa.adjustment_notes 'Adjustment Notes',month(accrued_date) 'Accrued Month',year(accrued_date) 'Accrued Year',poa.pre_accrual 'Pre-Accrual Flag' from po_transactions poa,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=poa.job_id and (poa.adjustment_code is null or poa.adjustment_code<>0) and (poa.accrued_date <='"+toDateTime+"' and poa.accrued_date >='"+fromDateTime+"') "+poaVendorFilter+" "+((showPreAccrual)?"":"and (poa.pre_accrual is null or poa.pre_accrual<>'1') ")+" order by poa.vendor_id,poa.accrued_date";
		subtotals=true;
}else if(stt.equals("m")){
	adjSQL="Select j.id 'K:H:',concat(month(accrued_date),'-',year(accrued_date)) 'S:Accrued Month-Year',poa.vendor_id 'Vendor #',poa.id 'Po:PO Adjustment #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(poa.adjustment_date,'%m/%d/%Y') 'Adjustment Date',DATE_FORMAT(poa.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',poa.job_id 'K:Job #',j.job_name 'Job Name',poa.adjustment_code 'Adj Code',poa.adjustment_amount 'T:R:Accrued Job Cost Adj',poa.adjustment_notes 'Adjustment Notes',month(accrued_date) 'Accrued Month',year(accrued_date) 'Accrued Year',poa.pre_accrual 'Pre-Accrual Flag' from po_transactions poa,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=poa.job_id and (poa.adjustment_code is null or poa.adjustment_code<>0) and (poa.accrued_date <='"+toDateTime+"' and poa.accrued_date >='"+fromDateTime+"') "+poaVendorFilter+" "+((showPreAccrual)?"":"and (poa.pre_accrual is null or poa.pre_accrual<>'1') ")+" order by poa.accrued_date";
	subtotals=true;
}else{
	adjSQL="Select j.id 'K:H:',poa.vendor_id 'Vendor #',poa.id 'Po:PO Adjustment #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(poa.adjustment_date,'%m/%d/%Y') 'Adjustment Date',DATE_FORMAT(poa.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',poa.job_id 'K:Job #',j.job_name 'Job Name',poa.adjustment_code 'Adj Code',poa.adjustment_amount 'T:R:Accrued Job Cost Adj',poa.adjustment_notes 'Adjustment Notes',month(accrued_date) 'Accrued Month',year(accrued_date) 'Accrued Year',poa.pre_accrual 'Pre-Accrual Flag' from po_transactions poa,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=poa.job_id and (poa.adjustment_code is null or poa.adjustment_code<>0) and (poa.accrued_date <='"+toDateTime+"' and poa.accrued_date >='"+fromDateTime+"') "+poaVendorFilter+" "+((showPreAccrual)?"":"and (poa.pre_accrual is null or poa.pre_accrual<>'1') ")+" order by poa.accrued_date";
