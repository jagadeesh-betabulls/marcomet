...
String reportTitle="Accrued Inventory Cost Report ";
...
if(excel){
	adjSQL="select j.dropship_vendor 'Vendor #',j.id  'Job #', DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Date Posted', j.brand_code 'Brand Code', j.product_code 'Product Code',  j.root_prod_code 'Root Product Code', j.job_name 'Job Name', j.quantity 'T:A:Quantity', j.price 'T:Price', j.shipping_price 'T:Shipping Price', j.accrued_po_cost 'T:Accrued PO Cost', j.accrued_shipping 'T:Accrued Shipping', j.accrued_inventory_cost 'T:Accrued Inv Cost', month(j.post_date) as 'R:Month Posted', year(j.post_date)as 'Year Posted' from jobs j where j.accrued_inventory_cost<>0 and j.accrued_inventory_cost is not null and j.post_date>='"+fromDate+"' and j.post_date<='"+toDate+"' order by j.post_date";
	subtotals=false;
}else if(stt.equals("m")){
	adjSQL="select concat(j.product_code,': ',month(j.post_date),'-', year(j.post_date)) 'S1:H:Month-Year Posted',j.product_code 'H:S:Product Code', concat(month(j.post_date),'-', year(j.post_date)) 'R:Month-Year Posted',  j.product_code 'Product Code', j.dropship_vendor 'Vendor #',j.id  'Job #', DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Date Posted', j.brand_code 'Brand Code', j.root_prod_code 'Root Product Code', j.job_name 'Job Name', j.quantity 'T:A:Quantity', j.price 'T:Price', j.shipping_price 'T:Shipping Price', j.accrued_po_cost 'T:Accrued PO Cost', j.accrued_shipping 'T:Accrued Shipping', j.accrued_inventory_cost 'T:Accrued Inv Cost' from jobs j where j.accrued_inventory_cost<>0 and j.accrued_inventory_cost is not null and j.post_date>='"+fromDate+"' and j.post_date<='"+toDate+"' order by j.product_code,j.post_date";
	subtotals=true;

}else{
	stt="p";
	adjSQL="select j.product_code 'S:Product Code', j.dropship_vendor 'Vendor #',j.id  'Job #', DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Date Posted', j.brand_code 'Brand Code', j.root_prod_code 'Root Product Code', j.job_name 'Job Name', j.quantity 'T:A:Quantity', j.price 'T:Price', j.shipping_price 'T:Shipping Price', j.accrued_po_cost 'T:Accrued PO Cost', j.accrued_shipping 'T:Accrued Shipping', j.accrued_inventory_cost 'T:Accrued Inv Cost', month(j.post_date) as 'R:Month Posted', year(j.post_date)as 'Year Posted' from jobs j where j.accrued_inventory_cost<>0 and j.accrued_inventory_cost is not null and j.post_date>='"+fromDate+"' and j.post_date<='"+toDate+"' order by j.product_code,j.post_date";
	subtotals=true;
}

%><!-- <%=adjSQL%> --><table id='invoice' class='sortable' width='100%' cellpadding=2><%
...
ArrayList st1Totals  = new ArrayList(15);
int hiddenCols=0;
String headerStr="<tr>";
String subTotCat="";
String subTot1Cat="";
for (int i=1 ;i <= numberOfAdjColumns; i++){
	...
	stTotals.add("0");	
	st1Totals.add("0");	
	tempString=str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(str.replaceSubstring(tempString,"A:", ""),"S1:", ""),"H:", ""),"S:", ""),"R:", ""),"T:", ""),"Po:", ""),"Pr:", ""),"K:", ""),"L:", ""),"~", "&nbsp;"),"C:", "");
	x++;
	if(tempString2.indexOf("H:")==(-1)){headerStr+="<td  class='text-row1' ><p>"+tempString+"</p></td>";}else{hiddenCols++;}
	subTot1Cat=((tempString2.indexOf("S:")==(-1))?subTot1Cat:tempString);
	subTotCat=((tempString2.indexOf("S1:")==(-1))?subTotCat:tempString);
}
headerStr+="</tr>";
String subTotStr="";
String subTot1Str="";
	...
	for (int i=0;i < numberOfAdjColumns; i++){
	   ...
	   if (showRow){
		 ...
		 if(subtotals && columnName.indexOf("S1:")>-1 && (!subTotStr.equals(((rsAdj.getString(columnName)==null)?"0":rsAdj.getString(columnName))) )){
			...
			addParams="";
			//((stt.equals("p"))?"vendorId="+subTotStr+"&dateTo="+toDate+"&dateFrom="+fromDate+"&vendorCompanyText="+subTotStr:"vendorId="+request.getParameter("vendorId")+"&dateTo="+(subTotStr.substring(subTotStr.indexOf("-")+1,subTotStr.length())+"-"+subTotStr.substring(0,subTotStr.indexOf("-"))+"-"+"31")+"&dateFrom="+(subTotStr.substring(subTotStr.indexOf("-")+1,subTotStr.length())+"-"+subTotStr.substring(0,subTotStr.indexOf("-"))+"-"+"01")+"&vendorCompanyText="+request.getParameter("vendorCompanyText"));
			if (i==0){%><%}

			
			
		}else if(subtotals && columnName.indexOf("S:")>-1 && !subTot1Str.equals(((rsAdj.getString(columnName)==null) ?"0":rsAdj.getString(columnName))) ){
			if(stRowCount>0){
				%><tr><%
				for (int j=0;j < numberOfAdjColumns; j++){
					String columnNameST = (String) headersAdj.get(j);
					String totalStrSt=((columnNameST.indexOf("T:")>-1)?((columnNameST.indexOf("A:")>-1)?dFormatter.format(Double.parseDouble((String)st1Totals.get(j))):decFormatter.format(Double.parseDouble((String)st1Totals.get(j)))):"&nbsp;");
					if ((j-hiddenCols)==0){
						%><td class='minderheaderright' align='right' colspan=<%=totHeadCol-hiddenCols%>><%=((totalsOnly)?"<a href=\"javascript:pop('/app-admin/payments/reports/AccruedInventoryXtionReport.jsp?"+addParams+"&siteText="+request.getParameter("siteText")+"&preaccrual="+((request.getParameter("preaccrual")==null)?"":request.getParameter("preaccrual"))+"','900','800')\" class='minderLink'>&raquo;</a>":"")%>TOTALS FOR <%=subTot1Cat+" "+subTot1Str%></td><%
					}else if (j>(totHeadCol-1)){
						%><td class='<%=((columnNameST.indexOf("T:")>-1)?"lineitemsright":"minderheaderright")%>' align='right'><b><%=totalStrSt%></b></td><%		
					}
				}
			...
			subTot1Str=((rsAdj.getString(columnName)==null)?"0":rsAdj.getString(columnName));
			addParams="";
			//((stt.equals("p"))?"vendorId="+subTotStr+"&dateTo="+toDate+"&dateFrom="+fromDate+"&vendorCompanyText="+subTotStr:"vendorId="+request.getParameter("vendorId")+"&dateTo="+(subTotStr.substring(subTotStr.indexOf("-")+1,subTotStr.length())+"-"+subTotStr.substring(0,subTotStr.indexOf("-"))+"-"+"31")+"&dateFrom="+(subTotStr.substring(subTotStr.indexOf("-")+1,subTotStr.length())+"-"+subTotStr.substring(0,subTotStr.indexOf("-"))+"-"+"01")+"&vendorCompanyText="+request.getParameter("vendorCompanyText"));
			if ((i-1)==0){%><%=headerStr%><%}
			...
			if (columnName.indexOf("T:")>-1){
				...
				double tempDST1=rsAdj.getDouble(columnName)+Double.parseDouble((String)st1Totals.get(i));				
				vTotals.set(i, Double.toString(tempD));
				stTotals.set(i, Double.toString(tempDST));
				st1Totals.set(i, Double.toString(tempDST1));
			}
			...
			if(!totalsOnly && columnName.indexOf("H:")==-1){
				%><td class='text-row<%=(((x % 2) == 0)?1:2)%>' align='<%=((columnName.indexOf("R:")>-1 ||columnName.indexOf("T:")>-1)?"right":((columnName.indexOf("C:")>-1)?"center":"left"))%>'><%=linkStr+((rsAdj.getString(columnName)==null)?"":((columnName.indexOf("T:")>-1)?((columnName.indexOf("A:")>-1)?dFormatter.format(Double.parseDouble(rsAdj.getString(columnName))):decFormatter.format(Double.parseDouble(rsAdj.getString(columnName)))):rsAdj.getString(columnName)) ) +((linkStr.equals(""))?"":"</a>")%></td><%
			}		
	 } //showrow
	} //for... columns
	x++;
...
if((stt.equals("p") ||stt.equals("m"))  && subtotals){
	%><tr><%
	for (int j=0;j < numberOfAdjColumns; j++){
		String columnNameST = (String) headersAdj.get(j);
		String totalStrSt=((columnNameST.indexOf("T:")>-1)?((columnNameST.indexOf("A:")>-1)?dFormatter.format(Double.parseDouble((String)st1Totals.get(j))):decFormatter.format(Double.parseDouble((String)st1Totals.get(j)))):"&nbsp;");
	...
	for (int i=0;i < numberOfAdjColumns; i++){
		String columnName = (String) headersAdj.get(i);
		String totalStr=((columnName.indexOf("T:")>-1)?((columnName.indexOf("A:")>-1)?dFormatter.format(Double.parseDouble((String)vTotals.get(i))):decFormatter.format(Double.parseDouble((String)vTotals.get(i)))):"&nbsp;");
		if (i==0){
