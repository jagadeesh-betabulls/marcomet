...
String reportTitle="AP Applied Transaction Report";
...
if(stt.equals("v")){
	adjSQL="Select j.id 'K:H:',apid.vendorid 'S:Vendor #',apid.vendor_invoice_no 'Po:Vendor Invoice #',DATE_FORMAT(apid.apinvoice_date,'%m/%d/%y') 'Invoice Date',DATE_FORMAT(apid.accrual_post_date,'%m/%d/%y') 'Accrual Post Date',apid.jobid 'K:Job #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',j.job_name 'Job Name',purchase_description 'Purchase Description',(apid.ap_purchase_amount) 'T:R:AP Purchase Amount',apid.ap_shipping_amount 'T:R:AP Shipping Amount',apid.ap_sales_tax_amount 'T:R:AP Sales Tax Amount',apid.amount 'T:R:AP Invoice Total Amount',month(apid.accrual_post_date) 'Accrued Month',year(apid.accrual_post_date) 'Accrued Year',apid.pre_accrual 'Pre-Accrual Flag' from ap_invoice_details apid,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=apid.jobid and (apid.apinvoice_date='0000-00-00' or (apid.accrual_post_date <='"+toDate+"' and apid.accrual_post_date >'"+fromDate+"')) "+apidVendorFilter+" "+((showPreAccrual)?"":" and (apid.pre_accrual is null or apid.pre_accrual<>'1') ")+" order by apid.vendorid, apid.apinvoice_date";	
		subtotals=true;
}else if(stt.equals("m")){
	adjSQL="Select j.id 'K:H:',concat(month(apid.accrual_post_date),'-',year(apid.accrual_post_date)) 'S:Accrual Post Month-Year',apid.vendorid 'Vendor #',apid.vendor_invoice_no 'Po:Vendor Invoice #',DATE_FORMAT(apid.apinvoice_date,'%m/%d/%y') 'Invoice Date',DATE_FORMAT(apid.accrual_post_date,'%m/%d/%y') 'Accrual Post Date',apid.jobid 'K:Job #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',j.job_name 'Job Name',purchase_description 'Purchase Description',(apid.ap_purchase_amount) 'T:R:AP Purchase Amount',apid.ap_shipping_amount 'T:R:AP Shipping Amount',apid.ap_sales_tax_amount 'T:R:AP Sales Tax Amount',apid.amount 'T:R:AP Invoice Total Amount',apid.pre_accrual 'Pre-Accrual Flag' from ap_invoice_details apid,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=apid.jobid and (apid.apinvoice_date='0000-00-00' or (apid.accrual_post_date <='"+toDate+"' and apid.accrual_post_date >'"+fromDate+"')) "+apidVendorFilter+" "+((showPreAccrual)?"":" and (apid.pre_accrual is null or apid.pre_accrual<>'1') ")+" order by apid.accrual_post_date, apid.apinvoice_date";
	subtotals=true;
}else{
	adjSQL="Select j.id 'K:H:',apid.vendorid 'Vendor #',apid.vendor_invoice_no 'Po:Vendor Invoice #',DATE_FORMAT(apid.apinvoice_date,'%m/%d/%y') 'Invoice Date',DATE_FORMAT(apid.accrual_post_date,'%m/%d/%y') 'Accrual Post Date',apid.jobid 'K:Job #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',j.job_name 'Job Name',purchase_description 'Purchase Description',(apid.ap_purchase_amount) 'T:R:AP Purchase Amount',apid.ap_shipping_amount 'T:R:AP Shipping Amount',apid.ap_sales_tax_amount 'T:R:AP Sales Tax Amount',apid.amount 'T:R:AP Invoice Total Amount',month(apid.accrual_post_date) 'Accrued Month',year(apid.accrual_post_date) 'Accrued Year',apid.pre_accrual 'Pre-Accrual Flag' from ap_invoice_details apid,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=apid.jobid and (apid.apinvoice_date='0000-00-00' or (apid.accrual_post_date <='"+toDate+"' and apid.accrual_post_date >'"+fromDate+"')) "+apidVendorFilter+" "+((showPreAccrual)?"":" and (apid.pre_accrual is null or apid.pre_accrual<>'1') ")+" order by apid.apinvoice_date";
		 ...
		 if(((stt.equals("v") && vendorNotChosen) || (stt.equals("m"))) && subtotals && columnName.indexOf("S:")>-1 && (!subTotStr.equals(((rsAdj.getString(columnName)==null)?"0":rsAdj.getString(columnName))) )){
			if(stRowCount>0){
				...
				for (int j=0;j < numberOfAdjColumns; j++){
					...
					if (j==0){
						%><td class='minderheaderright' align='right' colspan=<%=totHeadCol-hiddenCols%>><%=((totalsOnly)?"<a href=\"javascript:pop('/app-admin/payments/reports/APAppliedXtionReport.jsp?"+addParams+"&siteText="+request.getParameter("siteText")+"&preaccrual="+((request.getParameter("preaccrual")==null)?"":request.getParameter("preaccrual"))+"','900','800')\" class='minderLink'>&raquo;</a>":"")%>TOTALS FOR <%=((stt.equals(""))?"":((stt.equals("m"))?"ACCRUAL MONTH-YEAR "+subTotStr:"VENDOR "+ sl.getValue("vendors","id",subTotStr, "concat(notes,'[',id,']')"))) %></td><%
