...
String reportTitle="Ship Accrual Adjustments Transaction Report";
...
if(stt.equals("v")){
	adjSQL="Select j.id 'K:H:',sh.accrued_date 'H:Accrued' ,sh.shipping_vendor_id 'S:Vendor #',sh.id 'Po:Ship #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(sh.date,'%m/%d/%Y') 'Ship Date',DATE_FORMAT(sh.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',sh.job_id 'K:Job #',j.job_name 'Job Name',if(sh.adjustment_flag is null,0,sh.adjustment_flag) 'Adj Code',round(sh.subvendor_handling,2)  'T:R:Accrued Handling','0' as 'T:R:Accrued Ship Cost',round((sh.subvendor_handling+0),2) 'T:R:Total Shipping',month(accrued_date) 'Accrued Month',year(accrued_date) 'Accrued Year',sh.pre_accrual 'Pre-Accrual Flag' from shipping_data sh,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=sh.job_id and (sh.adjustment_flag=1) and (sh.accrued_date <='"+toDateTime+"' and sh.accrued_date >='"+fromDateTime+"') "+shVendorFilter+" "+((showPreAccrual)?"":" and (sh.pre_accrual is null or sh.pre_accrual<>'1') ");
	adjSQL+="union Select j.id 'K:H:',sh.accrued_date 'H:Accrued' ,sh.shipping_account_vendor_id 'S:Vendor #',sh.id 'Po:Ship #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(sh.date,'%m/%d/%Y') 'Ship Date',DATE_FORMAT(sh.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',sh.job_id 'K:Job #',j.job_name 'Job Name',if(sh.adjustment_flag is null,0,sh.adjustment_flag) 'Adj Code','0' as 'T:R:Accrued Handling',round(sh.cost,2) 'T:R:Accrued Ship Cost',round((0+sh.cost),2) 'T:R:Total Shipping',month(accrued_date) 'Accrued Month',year(accrued_date) 'Accrued Year',sh.pre_accrual 'Pre-Accrual Flag' from shipping_data sh,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=sh.job_id and (sh.adjustment_flag=1) and (sh.accrued_date <='"+toDateTime+"' and sh.accrued_date >='"+fromDateTime+"') "+shVendorFilter+" "+((showPreAccrual)?"":" and (sh.pre_accrual is null or sh.pre_accrual<>'1') ")+" order by `S:Vendor #`,`K:H:`, `Accrued Date`,`T:R:Accrued Ship Cost`";
		subtotals=true;
}else if(stt.equals("m")){
	adjSQL="Select j.id 'K:H:',sh.accrued_date 'H:Accrued' ,concat(month(accrued_date),'-',year(accrued_date)) 'S:Accrued Month-Year',sh.shipping_vendor_id 'Vendor #',sh.id 'Po:Ship #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(sh.date,'%m/%d/%Y') 'Ship Date',DATE_FORMAT(sh.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',sh.job_id 'K:Job #',j.job_name 'Job Name',if(sh.adjustment_flag is null,0,sh.adjustment_flag) 'Adj Code',round(sh.subvendor_handling,2)  'T:R:Accrued Handling','0' as 'T:R:Accrued Ship Cost',round((sh.subvendor_handling+0),2) 'T:R:Total Shipping',sh.pre_accrual 'Pre-Accrual Flag' from shipping_data sh,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=sh.job_id and (sh.adjustment_flag=1) and (sh.accrued_date <='"+toDateTime+"' and sh.accrued_date >='"+fromDateTime+"') "+shVendorFilter+" "+((showPreAccrual)?"":" and (sh.pre_accrual is null or sh.pre_accrual<>'1') ");
	
	adjSQL+="union Select j.id 'K:H:',sh.accrued_date 'H:Accrued' ,concat(month(accrued_date),'-',year(accrued_date)) 'S:Accrued Month-Year',sh.shipping_account_vendor_id 'Vendor #',sh.id 'Po:Ship #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(sh.date,'%m/%d/%Y') 'Ship Date',DATE_FORMAT(sh.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',sh.job_id 'K:Job #',j.job_name 'Job Name',if(sh.adjustment_flag is null,0,sh.adjustment_flag) 'Adj Code','0' as 'T:R:Accrued Handling',round(sh.cost,2) 'T:R:Accrued Ship Cost',round((0+sh.cost),2) 'T:R:Total Shipping',sh.pre_accrual 'Pre-Accrual Flag'from shipping_data sh,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where j.id=sh.job_id and (sh.adjustment_flag=1) and (sh.accrued_date <='"+toDateTime+"' and sh.accrued_date >='"+fromDateTime+"') "+shaVendorFilter+" "+((showPreAccrual)?"":" and (sh.pre_accrual is null or sh.pre_accrual<>'1') ")+" order by `H:Accrued`,`K:H:`,`T:R:Accrued Ship Cost`";
	subtotals=true;
}else{
	adjSQL="Select j.id 'K:H:',sh.accrued_date 'H:Accrued' ,sh.id 'H:',sh.shipping_vendor_id 'Vendor #',sh.id 'Po:Ship Adj #',sh.id 'Po:Ship #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(sh.date,'%m/%d/%Y') 'Ship Date',DATE_FORMAT(sh.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',sh.job_id 'K:Job #',j.job_name 'Job Name',if(sh.adjustment_flag is null,0,sh.adjustment_flag) 'Adj Code',round(sh.subvendor_handling,2)  'T:R:Accrued Handling','0' as 'T:R:Accrued Ship Cost',round((sh.subvendor_handling+0),2) 'T:R:Total Shipping', month(accrued_date) 'Accrued Month',year(accrued_date) 'Accrued Year',sh.pre_accrual 'Pre-Accrual Flag'from shipping_data sh,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where sh.subvendor_handling<>0 and j.id=sh.job_id and (sh.adjustment_flag=1) and (sh.accrued_date <='"+toDateTime+"' and sh.accrued_date >='"+fromDateTime+"') "+shVendorFilter+" "+((showPreAccrual)?"":" and (sh.pre_accrual is null or sh.pre_accrual<>'1') ");
	adjSQL+=" union Select j.id 'K:H:',sh.accrued_date 'H:Accrued' ,sh.id 'H:',sh.shipping_account_vendor_id 'Vendor #',sh.id 'Po:Ship Adj #',sh.id 'Po:Ship #',j.product_code 'Product Code',j.product_id 'Pr:Product ID',DATE_FORMAT(sh.date,'%m/%d/%Y') 'Ship Date',DATE_FORMAT(sh.accrued_date,'%m/%d/%Y') 'Accrued Date',DATE_FORMAT(j.post_date,'%m/%d/%Y') 'Job Accrued Date',sh.job_id 'K:Job #',j.job_name 'Job Name',if(sh.adjustment_flag is null,0,sh.adjustment_flag) 'Adj Code','0' as 'T:R:Accrued Handling',round(sh.cost,2) 'T:R:Accrued Ship Cost',round((0+sh.cost),2) 'T:R:Total Shipping',month(accrued_date) 'Accrued Month',year(accrued_date) 'Accrued Year',sh.pre_accrual 'Pre-Accrual Flag'from shipping_data sh,jobs j left join product_roots pr on j.root_prod_code=pr.root_prod_code  left join products p on j.product_id=p.id where sh.cost<>0 and j.id=sh.job_id and (sh.adjustment_flag=1) and (sh.accrued_date <='"+toDateTime+"' and sh.accrued_date >='"+fromDateTime+"') "+shaVendorFilter+" "+((showPreAccrual)?"":" and (sh.pre_accrual is null or sh.pre_accrual<>'1') ")+" order by `H:Accrued`,`K:H:`,`T:R:Accrued Ship Cost`";
}
		 ...
		 if(((stt.equals("v") && vendorNotChosen) || (stt.equals("m"))) && subtotals && columnName.indexOf("S:")>-1 && (!subTotStr.equals(((rsAdj.getString(columnName)==null)?"0":rsAdj.getString(columnName))) )){
			if(stRowCount>0){
				...
				for (int j=0;j < numberOfAdjColumns; j++){
					...
					if (j==0){
						%><td class='minderheaderright' align='right' colspan=<%=totHeadCol-hiddenCols%>><%=((totalsOnly)?"<a href=\"javascript:pop('/app-admin/payments/reports/ShipAccrualAdjustmentsXtionReport.jsp?"+addParams+"&siteText="+request.getParameter("siteText")+"&preaccrual="+((request.getParameter("preaccrual")==null)?"":request.getParameter("preaccrual"))+"','900','800')\" class='minderLink'>&raquo;</a>":"")%>TOTALS FOR <%=((stt.equals(""))?"":((stt.equals("m"))?"ACCRUAL MONTH-YEAR "+subTotStr:"VENDOR "+ sl.getValue("vendors","id",subTotStr, "concat(notes,'[',id,']')"))) %></td><%
