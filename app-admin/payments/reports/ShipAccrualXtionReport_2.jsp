...
if(!excel){%><jsp:include page="/app-admin/payments/APReportFilters_2.jsp" flush="true" >
		 ...
		 if(((stt.equals("v") && vendorNotChosen) || (stt.equals("m"))) && subtotals && columnName.indexOf("S:")>-1 && (!subTotStr.equals(((rsAdj.getString(columnName)==null)?"0":rsAdj.getString(columnName))) )){
			if(stRowCount>0){
				...
				for (int j=0;j < numberOfAdjColumns; j++){
					...
					if (j==0){
						%><td class='minderheaderright' align='right' colspan=<%=totHeadCol-hiddenCols%>><%=((totalsOnly)?"<a href=\"javascript:pop('/app-admin/payments/reports/ShipAccrualXtionReport_2.jsp?"+addParams+"&siteText="+request.getParameter("siteText")+"&preaccrual="+((request.getParameter("preaccrual")==null)?"":request.getParameter("preaccrual"))+"','900','800')\" class='minderLink'>&raquo;</a>":"")%>TOTALS FOR <%=((stt.equals(""))?"":((stt.equals("m"))?"ACCRUAL MONTH-YEAR "+subTotStr:"VENDOR "+ sl.getValue("vendors","id",subTotStr, "concat(notes,'[',id,']')"))) %></td><%
