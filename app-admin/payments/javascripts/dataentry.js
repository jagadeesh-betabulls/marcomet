...
function checkEntryFormData(){
	...
	if(selectedCompIndex == "0"){
		...
		var  orderObj=document.forms[0].orderNumber;
                if(invoiceObj.value == " " || jobObj.value == " " ||orderObj.value == " " )
                {
                    alert("Space not allowed !!!");
                    return false;
                }
		if(isNull(invoiceObj) && isNull(jobObj) && isNull(orderObj)){
			alert("Company/Job Number/Order Number/Invoice Number has to be entered.");
			return false;
		}
		if(!isNull(invoiceObj) && !isInteger(invoiceObj)&&!isNull(orderObj)){
