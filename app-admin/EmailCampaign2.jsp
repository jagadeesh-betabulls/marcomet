...
while (rs.next()){
	brandChoiceStr=rs.getString("b.id")+"|"+rs.getString("logo")+"|"+rs.getString("sh.site_host_name")+"|"+rs.getString("sh.domain_name")+"|"+rs.getString("site_host_id")+"|"+rs.getString("b.brand_code");
	slBrands+="<option value='"+brandChoiceStr+"' "+((brandChoiceStr.equals(brandChoice))?"Selected":"")+">"+rs.getString("b.brand_name")+"</option>";
}
slBrands+="</select>";
...
<form method="post" action="/app-admin/EmailCampaign2.jsp">
  ...
  <br><br><%
		if (step==2){
			...
			%><table width='768'><tr><td class="minderheader" colspan=2>EDIT HTML EMAIL MESSAGE</td></tr>
				<tr><td class=lineitem colspan=2><textarea name="html_area" cols='100' rows="35" ><jsp:include page="/includes/EmailFeaturedProducts.jsp" >
				...
				<table style="border: thin solid #000;padding: 0px;" cellpadding=0 cellspacing=0><tr><td><jsp:include page="<%="/includes/topmast_email.jsp"%>" ><jsp:param name="siteHostName" value="<%=site_host_name%>" /><jsp:param name="sitehostroot" value="<%="http://"+domain_name+"/sitehosts/"+site_host_name%>" /></jsp:include></td></tr><tr><td style="padding: 4px;"><span id='testarea'></span></td></tr><tr><td class='lineitems'>To view this offer and explore all your marketing options go to: <a href='<%="http://"+domain_name+"/index.jsp?contents=/contents/Promopage.jsp?cId1="+campaignId%>'><%="http://"+domain_name+"/index.jsp?contents=/contents/Promopage.jsp?cId1="+campaignId%></a></td></tr></table><script>popTest();</script><%
...
int last=0;
	while(true){
		first=plainText.indexOf("<");
			last=plainText.indexOf(">");
			if(first>-1 && last >-1){
				plainText=plainText.substring(0, first)+plainText.substring(last+1, plainText.length());
			}else{
				break;
			}
		}
							%><textarea name="plaintext" cols='100' rows="35" ><%=plainText%></textarea><%
