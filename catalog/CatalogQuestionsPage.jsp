...
<link rel="stylesheet" href="/styles/misc_styles.css?v1" type="text/css">

<link rel="stylesheet" href="/styles/modalbox.css" type="text/css">

<script language="JavaScript" src="/javascripts/mainlib.js?v1"></script>
...
String templateCategory=((request.getParameter("templateCategory")==null)?"":request.getParameter("templateCategory"));
...
String floatPreview=sl.getValue("products","id","'"+productId+"'","p_float_preview");
...
String showRetailTable=sl.getValue("site_hosts","id",siteHostId,"show_retail_grid");
...
if (rsDescription.next()){
	...
	pageDescription=((rsDescription.getString("description")==null)?"":"<tr><td class='body' valign='top'>"+((floatPreview.equals("1"))?"<div class='floatCatalogText'>":"")+rsDescription.getString("description")+((floatPreview.equals("1"))?"</div>":"")+"<br></td></tr>");
...
if (request.getParameter("productId")!=null && !request.getParameter("productId").equals("") && !catSpecIdNum1.equals("")){
	...
	if (rsProdInfo.next()){
		...
		templateCategory=((rsProdInfo.getString("initiative")==null || rsProdInfo.getString("initiative").equals(""))?templateCategory:rsProdInfo.getString("initiative"));
		...
		floatPreview=((rsProdInfo.getString("p_float_preview")==null || rsProdInfo.getString("p_float_preview").equals("") )?"0":rsProdInfo.getString("p_float_preview"));
...
<div align="left">
                ...
                <p class="bodyBlack" ><%=((floatPreview.equals("1"))?"<div class='floatPreview'>":"")%><span class="catalogLABEL"><%
...
<%=((shipPolicy.equals("") || allowPrintFile)?"":"<div class='subtitle' style='color:red;'>NOTE: "+shipPolicy+"")%>
            ...
            <%=((rsProdInfo.getString("catalog_text")==null)?"":"<div class=subtitle><br>"+rsProdInfo.getString("catalog_text")+"")%>
                  ...
                  </p><%=((floatPreview.equals("1"))?"</div>":"")%></div><%if(postProdCost){%><div class='subtitle'>Please note: pricing for this job will be determined upon job completion based on time and expense for production.</div><%}%></td>
...
//check for multiple property assignments. If true, show dropdown.

Integer numProps=1;

if (usePreview){

	String sql = "SELECT count(id) qcount  FROM contact_property_bridge cpb where contact_id='"+contactId+"'";

	ResultSet rsCPCount=st.executeQuery(sql);

	if(rsCPCount.next()){

   		numProps=((rsCPCount.getInt("qcount")<1)?1:rsCPCount.getInt("qcount"));
...
%><span class='catalogLABEL'>Select the Property Information to be Used for this job:</span><br>(NOTE: The job will be billed and sent to the primary property associated with your account.)<br><%

	String sqlP="Select distinct a.* from (SELECT concat('Site: ',wp.site_number,',',wp.site_name,',',wp.address1) as propname, wp.site_number,wp.site_name,wp.address1,wp.address2,wp.city,wp.state,wp.zip,wp.phone,wp.fax FROM contact_property_bridge cpb left join wyndham_properties wp on wp.site_number=cpb.site_number where contact_id='"+contactId+"' and wp.Site_status='7' union SELECT concat('Site: ',wp.site_number,',',wp.site_name,',',wp.address1) as propname, wp.site_number,wp.site_name,wp.address1,wp.address2,wp.city,wp.state,wp.zip,wp.phone,wp.fax FROM contact_property_bridge cpb left join contacts ct on ct.id = cpb.contact_id left join wyndham_properties wp on wp.site_number=ct.default_site_number where contact_id='"+contactId+"' and wp.Site_status='7' ) as a";

	Integer p=0;

	ResultSet rsCProps = st.executeQuery(sqlP);

	String options="";

	while (rsCProps.next()) {

		options+="<option value='"+p.toString()+"'>"+rsCProps.getString("propname")+"</option>";

		vProps.addElement(rsCProps.getString("propname"));

		vProps.addElement(rsCProps.getString("site_number"));

		vProps.addElement(rsCProps.getString("site_name"));

		vProps.addElement(rsCProps.getString("address1"));

		vProps.addElement(rsCProps.getString("address2"));

		vProps.addElement(rsCProps.getString("city"));

		vProps.addElement(rsCProps.getString("state"));

		vProps.addElement(rsCProps.getString("zip"));

		vProps.addElement(rsCProps.getString("phone"));

		vProps.addElement(rsCProps.getString("fax"));

		p=p+1;

	}

	%><select onchange="updatePreview(this,'buildTempl.jsp')" id="siteChoice" name="siteChoice"><%=options%></select><%
   ...
   if(rsQCount.next()){
      ...
      sql = "SELECT cs.id, spec_id, cs.axis, ls.value AS spec_title, if(ctb.field_label is not null and ctb.field_label<>'',ctb.field_label,label) as label, if(ctb.content_catspecid>0,716,if(ctb.field_type>0,ctb.field_type,cs.field_type)) as field_type, reset_value, include_pic_url,if(text_case='upper',concat('upper(',if(ctb.preload_sql is not null and ctb.preload_sql<>'',ctb.preload_sql,preload_field),')'),if(text_case='title',concat('tcase(',if(ctb.preload_sql is not null and ctb.preload_sql<>'',ctb.preload_sql,preload_field),')'),if(ctb.preload_sql is not null and ctb.preload_sql<>'',ctb.preload_sql,preload_field))) as preload_field,ctb.text_case,ctb.page as pdf_page,image_height_inches,if(image_height_inches is not null,image_height_inches*72,300) as image_height_pixels,image_width_inches,if(image_width_inches is not null,image_width_inches*72,300) as image_width_pixels,pdfblock_name,entry_hints,image_dpi,js.value as specValue,ctb.default_contents as defaultSpecValue,ctb.text_case as textCase,if(content_catspecid is not null and content_catspecid>0,0,ctb.show_field) as showField  FROM catalog_specs cs left join catspec_template_bridge ctb on ctb.page>0 and ctb.catspecid=cs.id and template_name='"+pdfPreviewTemplate+"'  left join job_specs js on js.cat_spec_id=cs.id and js.job_id='"+priorJobId+"', lu_specs ls WHERE "+((usePreview)?"ctb.id is not null AND ":"")+" cat_job_id = " + request.getParameter("catJobId") + " AND cs.page = " + pageNumber + " AND ls.id = cs.spec_id AND (axis = 724 or axis=725) ORDER BY ctb.sequence,cs.sequence";
...
//   		String textCase=((rsQuestions.getString("text_case")==null)?"":rsQuestions.getString("text_case"));
   		...
   		if (!pdfPage.equals("") && !pdfPage.equals(priorPage)){

   			%><div class='offeringTITLE'><br>PAGE <%=pdfPage%><br></div><%
		...
		if (fieldType == 715) { //Text
		...
		} else if (fieldType == 732) { //Locked Text

			if (showField && hPreProp.get(Integer.toString(specId))==null && request.getParameter(Integer.toString(specId))==null){

				%><%=catQuestionLabel%><input readonly class='readonlyText' tabindex="<%=tabIndex++%>" type="text" size="40" id="<%=((rsQuestions.getString("preload_field") != null && !(rsQuestions.getString("preload_field").equals("")))?rsQuestions.getString("preload_field"):tabIndex)%>" <%=((usePreview)?"onFocus=\"  lastFieldValue=this.value; \" onBlur=\" updatePreview(this,'"+previewTemplate+"')\" ":"") %>name="^<%=specId%>^<%=catSpecId%>" value="<%=existingSpecValue%>"><%
		...
		} else if (fieldType == 733) { //Hidden Text
			...
			if (showField && hPreProp.get(Integer.toString(specId))==null && request.getParameter(Integer.toString(specId))==null){
				...
				%><br><br><a href="javascript:updateImage('','<%=templateCategory%>','http://<%=baseURL%>','<%=((rsQuestions.getString("preload_field") != null && !(rsQuestions.getString("preload_field").equals("")))?rsQuestions.getString("preload_field"):"^"+specId+"^"+catSpecId)%>'<%
...
try{
         ...
         if (fieldType == 719) { //Radio Button
            ...
            while (rsQuestionChoices.next()) {
			   ...
			   if (tempValue.equals(existingSpecValue)) { %><%=catQuestionLabel%><input tabindex="<%=tabIndex++%>" type="radio"  <%=((usePreview)?"onFocus=\"  lastFieldValue=this.value; \" onClick=\" updatePreview(this,'"+previewTemplate+"')\" ":"") %>  id="<%=((rsQuestions.getString("preload_field") != null && !(rsQuestions.getString("preload_field").equals("")))?rsQuestions.getString("preload_field"):tabIndex)%>" name="^<%=specId%>^<%=catSpecId%>" value="<%=tempValue%>" checked><%= (picUrl == null || picUrl.equals("")) ? tempValue:picUrl %><%

               } else { 

					%><input tabindex="<%=tabIndex++%>" type="radio"  <%=((usePreview)?"onFocus=\"  lastFieldValue=this.value; \" onClick=\" updatePreview(this,'"+previewTemplate+"')\" ":"") %> id="<%=((rsQuestions.getString("preload_field") != null && !(rsQuestions.getString("preload_field").equals("")))?rsQuestions.getString("preload_field"):tabIndex)%>" name="^<%=specId%>^<%=catSpecId%>" value="<%=tempValue%>"><%= (picUrl == null || picUrl.equals("")) ? tempValue:picUrl %><%
			...
			%><%=catQuestionLabel%><select tabindex="<%=tabIndex++%>"  <%=((usePreview)?"onFocus=\"  lastFieldValue=this.value; \" onChange=\" updatePreview(this,'"+previewTemplate+"')\" ":"") %> id="<%=((rsQuestions.getString("preload_field") != null && !(rsQuestions.getString("preload_field").equals("")))?rsQuestions.getString("preload_field"):tabIndex)%>" name="^<%=specId%>^<%=catSpecId%>"><%
            ...
            while (rsQuestionChoices.next()) {
				...
				ResultSet rs3 = st4.executeQuery(query3);
			...
			%></select><%
				...
				%><%=catQuestionLabel%><input type="checkbox"  <%=((usePreview)?"onFocus=\"  lastFieldValue=this.value; \" onChange=\" updatePreview(this,'"+previewTemplate+"')\" ":"") %> id="<%=((rsQuestions.getString("preload_field") != null && !(rsQuestions.getString("preload_field").equals("")))?rsQuestions.getString("preload_field"):tabIndex)%>" name="^<%=rsQuestions.getString("spec_id")%>^<%=catSpecId%>" value="<%=existingSpecValue%>"><%=existingSpecValue%><%
	...
	%></div></td><%if(usePreview && qCount++==0){%><td valign="top"><div id='previewD'><div id='previewProcessing'>Your sample image is being processed, and will appear below...</div>

	<iframe height="<%=((Integer.parseInt(previewHeight)<400 || floatPreview.equals("0"))?previewHeight:"85%")%>" width="<%=previewWidth%>" marginwidth="0" marginheight="0" frameborder="0" id="preview" name="preview" <%=((floatPreview.equals("1"))?"class='floatPreview'":"")%> ></iframe><input type="hidden" name="imageusername" id="imageusername" value='<%=imageUserName%>'></div></td><%}%></tr><%
	...
	System.out.println("cSQL is "+cSQL);
...
<% if(allowPrintFile && currentCatalogPage.equals("1") && !reprintFile){
          ...
          <tr>
			...
			%><%=((showRetailTable.equals("1"))?"<br><a href=\"javascript:pop('/contents/retailTable.jsp?productId="+productId+"','650','80')\" class='menuLINK'>Click here for standard retail price list…</a><br><br>":"")%><taglib:PricingGridTag productCode="<%=prodCode%>" catJobId="<%=catJobID%>" contactId="<%=contactId%>" vendorId="<%=vendorId%>" tierId="<%=tierId%>" siteHostId="<%=siteHostId%>" catalogPage="<%=pageNumber%>" proxyEnabled="<%=proxyEnabled%>"/><%
 ...
 <!--       </td>
    ...
    <tr> 
      ...
      <td valign="top" align="center" colspan="2" valign="top"> -->
        ...
        <div align='center' id="footerDiv" <%=((usePreview && floatPreview.equals("1"))?"class='floatCatalogText'":"")%> ><%
...
function hidePreviewProcessing(){

	var el= document.getElementById("previewProcessing");
...
if(!priorJobId.equals("") ) {
	...
	if(gridExists.equals("0")){
		...
		String lastRow = session.getAttribute("lastRow").toString();

		String lastCol = session.getAttribute("lastCol").toString();
