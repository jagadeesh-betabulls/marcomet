...
<%
    ...
    String storeShipLocation = ((request.getParameter("storeShipLocation")==null)?"0":request.getParameter("storeShipLocation"));
	...
	String subvendorReferenceData=((session.getAttribute("subvendorReferenceData")==null)?"":session.getAttribute("subvendorReferenceData").toString());
	String customerPO=((session.getAttribute("customerPO")==null)?"":session.getAttribute("customerPO").toString());	
...
//      if it does, apply that , else save it as a new shipping_location record and apply it to the shopping cart.
		...
		if (projects0.size() > 0) {
			for (int jjj = 0; jjj < projects0.size(); jjj++) {
	  			...
	  			for (int kkj = 0; kkj < jobs0.size(); kkj++) {
	    			...
	    			jo0.setPromoCode("");
	    			jo0.setSubvendorReferenceData(subvendorReferenceData);
    	...
    	if (!promoCode.equals("")){
    	    ResultSet rsPromoSH = st.executeQuery("Select * from promo_sitehost_bridge where promo_code='"+promoCode+"' and exclude_sitehost=0");
	 		...
	 		if (excludeSitehostFromPromo){
		 		...
		 		ResultSet rsPromoSH3 = st.executeQuery("Select * from promo_customer_bridge where promo_code='"+promoCode+"' and (site_number='"+up.getSiteNumber()+"' or company_id='"+up.getCompanyId()+"' or tax_id='"+up.getTaxId()+"')");
	 			if(rsPromoSH3.next()){
	...
	if (promoStatus.equals("Valid")){
    	...
    	if(request.getParameter("changeShipAddress")!=null && request.getParameter("changeShipAddress").equals("true")){
    		...
    		String newAddr="insert into shipping_locations ( address2, address1, state, zip, country_id, contactid, location_title, city, ship_to_name,companyid,reuse_address) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    		...
    		cAddr.setString(11, request.getParameter("storeShipLocation"));
    	 	cAddr.executeUpdate();
...
*/
    ...
    %><div class="winBox"><div class="offeringTITLE" align="center">There are no items in your shopping cart. Please click continue to return home.<br><br><a target="_top" href="http://<%=(String)session.getAttribute("baseURL")%>" class="menuLINK" >CONTINUE&nbsp;SHOPPING</a></div></div><%
    }
...
%><div class="title" id="title">Order Review / Checkout</div><!-- - Site: <%= up.getSiteNumber() %> Company: <%= up.getCompanyId() %> Tax id: <%= up.getTaxId() %>--><%
...
<div align="center"  id="processButtons">
		...
		<a target="_top" href="http://<%=(String)session.getAttribute("baseURL")%>" class="menuLINK" >CONTINUE&nbsp;SHOPPING</a>
		&nbsp;&nbsp;&nbsp;
...
<%if(promoStatus.equals("Valid")){%><td class="lineitems"><div align="right"><%= formater.getCurrency(shoppingCart.getDiscount())%></div></td><%}%>
       ...
       if(sl.getValue("site_hosts","id",shs.getSiteHostId(),"hide_shipping_options").equals("0")){
          ...
          <tr>
            <td>&nbsp;</td>
            <td colspan="5">
            <table border=0><tr><td valign="top"><%if (session.getAttribute("customerPO") == null || session.getAttribute("customerPO").toString().equals("")) {
	            	%><a href="javascript:pop('/popups/customerPO.jsp','250','150')" class='graybutton'  style="padding:2px;font-size:10pt;">&nbsp;&raquo;&nbsp;Apply&nbsp;Customer&nbsp;Reference/PO&nbsp;Number&nbsp;</a><br><br><%
	        } else {
	            	%><a href="javascript:pop('/popups/customerPO.jsp','250','150')" class='graybutton'  style="padding:2px;font-size:10pt;">&nbsp;&raquo;&nbsp;Change&nbsp;Customer&nbsp;Reference/PO&nbsp;Number&nbsp;</a><br><br><span align="right" id="customerPO" class='lineitems'>Customer&nbsp;Reference/PO&nbsp;Number:&nbsp;<%=customerPO%><br><%
	        }%></td><td>&nbsp;&nbsp;&nbsp;</td>
	        <td valign="top"><%if(showPromoDetails){
	            if (session.getAttribute("promoCode") == null || session.getAttribute("promoCode").toString().equals("")) {
	            	%><a href="javascript:pop('/popups/PromoCode.jsp','250','150')" class='graybutton'  style="padding:2px;font-size:10pt;">&nbsp;&raquo;&nbsp;Apply&nbsp;Promo&nbsp;Code&nbsp;</a><%
	            } else {
	            	%><a href="javascript:pop('/popups/PromoCode.jsp','250','150')" class='graybutton'  style="padding:2px;font-size:10pt;">&nbsp;&raquo;&nbsp;Change&nbsp;Promo&nbsp;Code&nbsp;</a><br><br><span align="right" id="promoCode" class='lineitems'>Promo Code:&nbsp;<%=session.getAttribute("promoCode").toString()%><br><%=promoDescription%><br>&nbsp;
	        <%
