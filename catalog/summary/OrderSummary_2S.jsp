...
<form method="post" action="/servlet/com.marcomet.sbpprocesses.ProcessJobSubmit" >
  <table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
    ...
    if (shoppingCart != null) {
      ...
      /* New code...spanchal
       * In the new code, the Jobs are being captured from the ShipmentObject
       * instead of ProjectObject as in the above suppressed code.
       */
      ...
      if (shipments.size() > 0) {
        for (int j = 0; j < shipments.size(); j++) {
          ...
          <tr>
            ...
            <td class="lineitems">
              <%-- As per the new code, Jobs are stored inside the Shipment, so it cannot be removed from Project as below... spanchal --%>
            ...
            %>
            <td class="lineitems">
              <%--<div align="right"><%= (shipPolicy.equals("0")) ? ((joShippingCost == 0.00) ? "TBD *" : formater.getCurrency(joShippingCost)) : formater.getCurrency(joShippingCost)%><%if (shipPolicy.equals("0") && joShippingCost != 0.00) {%>&nbsp;<a href="#" target="_self" class="minderACTION" onclick="openChangeShipTypePopup('<%= shippingType%>', <%= jobCount%>, <%= new DecimalFormat("#0.00").format(shipper.getGroundCost() + shipper.getHandlingCost(price, shipper.getGroundCost()))%>, <%= new DecimalFormat("#0.00").format(shipper.getThreeDayCost() + shipper.getHandlingCost(price, shipper.getThreeDayCost()))%>, <%= new DecimalFormat("#0.00").format(shipper.getSecondDayCost() + shipper.getHandlingCost(price, shipper.getSecondDayCost()))%>, <%= new DecimalFormat("#0.00").format(shipper.getNextDayCost() + shipper.getHandlingCost(price, shipper.getNextDayCost()))%>)">change</a><%}%></div>--%>
              <div align="right"><%= (shippingCalculated) ? ((joStdShippingPrice == 0.00) ? "TBD *" : formater.getCurrency(joStdShippingPrice)) : "$0.00"%></div>
            </td>
            <%}%>
          ...
          %>
          <tr>
            <td class="lineitems" colspan="5" align="right">
              <a href="#" target="_self" class="menuLink" onclick="javascript:window.location.href='/catalog/summary/OrderSummary.jsp?shippingCalculated=true'; AjaxModalBox.open($('divWait'), {width : 200, height : 100});">&nbsp;Calculate Shipping&nbsp;</a>
            </td>
          </tr>
          <%}
          %>
  ...
  if (creditStatus != 2 && overLimit) {
          ...
          %></tr>
          <tr><%if (((String) session.getAttribute("siteHostRoot")).equals("/sitehosts/network")) {
...
//if this is called from the 'Process Jobs' button don't use the footer
  ...
  <table width="100%" align="center" class="body">
    ...
    <tr>
      ...
      <td class="lineitems" align="right"><%= so.getShippingCost()%> (from UPS) + <%= so.getHandlingCost()%> Handling Cost (SubFee + MCFee) = <%= so.getShippingCost() + so.getHandlingCost()%></td>
    ...
    function toggleShipType(shipType){
      if(shipType == "ground"){
        if(AjaxModalBox.options.shippingType != 'ground'){
          window.location.href = '/catalog/summary/OrderSummary.jsp?jobNo='+AjaxModalBox.options.jobNo+'&shipType=ground';
        }
        AjaxModalBox.close();
      }
      else if(shipType == "nextday"){
        if(AjaxModalBox.options.shippingType != 'nextday'){
          window.location.href = '/catalog/summary/OrderSummary.jsp?jobNo='+AjaxModalBox.options.jobNo+'&shipType=nextday';
        }
        AjaxModalBox.close();
      }
      else if(shipType == "secondday"){
        if(AjaxModalBox.options.shippingType != 'secondday'){
          window.location.href = '/catalog/summary/OrderSummary.jsp?jobNo='+AjaxModalBox.options.jobNo+'&shipType=secondday';
        }
        AjaxModalBox.close();
      }
      else if(shipType == "threeday"){
        if(AjaxModalBox.options.shippingType != 'threeday'){
          window.location.href = '/catalog/summary/OrderSummary.jsp?jobNo='+AjaxModalBox.options.jobNo+'&shipType=threeday';
        }
        AjaxModalBox.close();
      }
    }
