...
<form method="post" action="/servlet/com.marcomet.sbpprocesses.ProcessJobSubmit" >
  <script type="text/javascript">
    function setShipmentOption(option) {
      var option1 = document.getElementById("shipOption1");
      var option2 = document.getElementById("shipOption2");
  ...
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
    <tr valign="top">
      <td colspan="10" height="155">
        <table width="50%" align="center" class="body">
          <tr>
            <p>
          ...
          <tr>
            ...
            <td class="planheader1" width="39%" height="9">Item / Job Name (Click to view Details)</td>
    ...
    <tr valign="middle" align="center">
      <td>
        <input name="shipmentOption" value="0" type="radio" onclick="setShipmentOption(0)" checked>&nbsp;Hold and Ship items together where possible to minimize shipping cost
        &nbsp;
        <input name="shipmentOption" value="1" type="radio" onclick="setShipmentOption(1)">&nbsp;Ship items as they become available
      </td>
    </tr>
    <%if (ordersPresent) {
    %>
  ...
  if ((creditStatus != 1 && validprop == 1 && proppastdue != 1) || creditStatus == 2 || session.getAttribute("proxyId")!=null) {
            %><td align="center"><a href="javascript:parent.window.location.replace('/index.jsp?contents=<%=(String) session.getAttribute("siteHostRoot")%>/contents/SiteHostIndex.jsp')" class="menuLINK">Continue&nbsp;Shopping</a></td><%
            } else {
            %><td align="center">&nbsp&nbsp&nbsp</td><%        }
  }
  if (session.getAttribute("proxyId")!=null || (session.getAttribute("demo") == null && ((validprop == 1 && proppastdue != 1 && creditStatus != 1 && overLimit == false) || creditStatus == 2))) {%>
            <td align="left"><span id="shipOption1" style="display:inline;"><a href="/catalog/summary/OrderSummary2.jsp?shipmentOption=1" class="menuLINK">Continue&nbsp;to&nbsp;Checkout</a></span><span id="shipOption2" style="display:none;"><a href="/catalog/summary/OrderSummary2.jsp?shipmentOption=2" class="menuLINK">Continue&nbsp;to&nbsp;Checkout</a></span></td>
            <%         } else {%>
            <td align="center"><a href="/" class="menuLINK">Home</a></td><%=((session.getAttribute("demo") == null) ? "" : "</tr><tr><td align=center colspan=3 class=title>[ DEMO SITE ]</td>")%><%
  }
          %></tr>
        </table>
      </td>
    ...
    <tr>
      <td valign="bottom" colspan="100%" align="center">
        <table><%=(((proppastdue + pastdue) > 0) ? "<tr><td colspan=3>Balance Over " + co_days + " days for all accounts on this property: " + formater.getCurrency(propInvoiceBalance) + "</td></tr>" : "")%><%=(((proppastdue + pastdue) > 0) ? "<tr><td colspan=3>Balance Over " + co_days + " days for this account: " + formater.getCurrency(invoiceBalance) + "</td></tr>" : "")%>
          <tr>
            <td align="right"><a href="javascript:parent.window.location.replace('/index.jsp?contents=<%=(String) session.getAttribute("siteHostRoot")%>/contents/SiteHostIndex.jsp')" class="menuLINK">Continue&nbsp;Shopping</a></td>
            <td>&nbsp;</td>
            <td align="left"><a href="/" class="menuLINK">Home</a></td>
          </tr>
        </table>
      </td>
    </tr>
