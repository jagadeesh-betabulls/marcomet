...
%><table width="98%" class="body">
    ...
    if (shoppingCart != null) {
      ...
      if (shipments.size() > 0) {
        for (int j = 0; j < shipments.size(); j++) {
          ...
          <tr>
            <td class="lineitems">
              <%--div align="center"><a href="/catalog/summary/RemoveJobConfirmation.jsp?jobId=<%= jo.getId()%>" class="minderACTION">delete</a></div>--%><div align="center">
            <a href="/catalog/summary/RemoveProjectConfirmation.jsp?projectId=<%= projectId + ""%>&confirm=true&shipmentOption=<%= shipmentOption + ""%>" class="minderACTION"><img src="/images/delete.jpg" border="0" alt="Remove Job from Shopping Cart"></a></div></td>
            <td class="lineitems">
          ...
          %><tr>
            ...
            <td class="lineitems"><div align="right"><%= formater.getCurrency(shoppingCart.getDiscount())%></div></td>
    ...
    if (shipmentOption.equals("1")) {
          ...
          <%} else if (shipmentOption.equals("2")) {
          %>
          <tr align="center">
            <td colspan="8">
              &nbsp;&nbsp;&nbsp;&nbsp;<input name="shipmentOption" value="0" type="radio" onclick="if(this.checked) {parent.window.location.replace('/catalog/summary/OrderSummary2.jsp?shipmentOption=1');}">&nbsp;Hold and Ship items together where possible to minimize shipping cost
              <br>
              &nbsp;&nbsp;&nbsp;&nbsp;<input name="shipmentOption" value="1" type="radio" onclick="if(this.checked) {parent.window.location.replace('/catalog/summary/OrderSummary2.jsp?shipmentOption=1');}" checked>&nbsp;Ship items as they become available
            </td>
          </tr>
          <%}%>
...
<%
  ...
  if (creditStatus != 2 && overLimit) {
            ...
            %><td colspan = 3 class="errorbox" >Online ordering has been temporarily suspended until your overdue invoices are paid.<br>Please click "home" to go back to the home page, then click on your overdue invoices listed on the lower left to pay immediately by credit card and complete your order, or to print them and pay by check.  If you need immediate assistance, please click "contact info" below to reach Marketing Services. You may place the current order by paying for these items with a credit card.<br><br>
              <a class="menuLINK" href="javascript:pop('<%=(String) session.getAttribute("siteHostRoot")%>/contents/contacts.jsp','650','450')">[CONTACT INFO]</a>
            </td><%

  } else if (creditStatus != 2 && proppastdue == 1) {
            %> <td colspan = 3 class="errorbox" >Online ordering for your account has been temporarily suspended for overdue invoices from other accounts related to your property.<br>Please click "contact info" below to reach Marketing Services so that we may resolve your account. You may place the current order by paying for these items with a credit card.<br><br>
              <a  class="menuLINK"  href="javascript:pop('<%=(String) session.getAttribute("siteHostRoot")%>/contents/contacts.jsp','650','450')">[CONTACT INFO]</a>
            </td> <%
...
//End Job Summary Table      
      if (creditStatus < 2 && (pastdue == 1 || proppastdue == 1 || creditStatus==1)){
      		%><input type="hidden" name="pay_type" value="cc"><%
      }else{
      		%><br><div id="Payment" class="body"><b>Please choose your payment option:</b><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="pay_type" value="account" onclick="document.getElementById('ach').style.display='none';document.getElementById('cc').style.display='none';" checked>On Account<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="pay_type" value="cc" onclick="document.getElementById('cc').style.display='';document.getElementById('ach').style.display='none';">Credit Card<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="pay_type" value="ach" onclick="document.getElementById('ach').style.display='';document.getElementById('cc').style.display='none';">Electronic Check (ACH)<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="pay_type" value="check" onclick="document.getElementById('ach').style.display='none';document.getElementById('cc').style.display='none';">Check to be Sent with Order</div><%
      }%>
      <!-- Begin Process CC -->
    <div id="cc" style="display:none;"><jsp:include page="/includes/ProcessCredCard.jsp" flush="true"></jsp:include></div>
      <!-- End Process CC -->
      <!-- Begin Process ACH -->
    <div id="ach" style="display:none;"><jsp:include page="/includes/ProcessACH.jsp" flush="true"></jsp:include></div>
      <!-- End Process ACH -->
      </td>
      ...
      <td width="60%">
            <!-- Begin Ship Info -->
              <jsp:include page="/includes/OrderCheckOutFormInc.jsp" flush="true">
                <jsp:param name="shipmentOption" value="<%=shipmentOption%>"></jsp:param>
              </jsp:include>
           <!-- End Ship Info -->
        </td>
    </tr>
        ...
        <table width="50%" align="center">
          <tr>
            <td align="right" width="48%">
              <div align="right"><a href="javascript:parent.window.location.replace('/index.jsp?contents=<%=(String) session.getAttribute("siteHostRoot")%>/contents/SiteHostIndex.jsp')" class="menuLINK">Continue&nbsp;Shopping</a></div>
            </td>
            <td width="4%">&nbsp;</td>
            <td align="left" width="48%">
              <div align="left"><a href="javascript:processOrder()" class="menuLINK">Submit&nbsp;Order</a></div>
            </td>
          </tr>
        </table>
        <script>
  ...
  <%}%>    
    ...
    %><tr>
      <td valign="bottom" colspan="100%" align="center">
        <table><%=(((proppastdue + pastdue) > 0) ? "<tr><td colspan=3 class='body'>Balance Over " + co_days + " days for all accounts on this property: " + formater.getCurrency(propInvoiceBalance) + "</td></tr>" : "")%><%=(((proppastdue + pastdue) > 0) ? "<tr><td colspan=3 class='body'>Balance Over " + co_days + " days for this account: " + formater.getCurrency(invoiceBalance) + "</td></tr>" : "")%>
          <tr>
