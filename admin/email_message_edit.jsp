...
String sql="";
String imagePath="/home/htdocs/salestrack"+(String)session.getAttribute("siteHostRoot")+"/fileuploads/page_images/";
...
<form method="post" action="/admin/email_message_edit.jsp?type=save&messageId=<%=messageId%>">
...
%><table><tr><td class="minderheader">EMail Key</td>
	...
	while (rs.next()){
		%><tr><td class=lineitem><a href="/admin/email_message_edit.jsp?type=edit&messageId=<%=rs.getString("id")%>"><%=rs.getString("email_key")%></a></td><td><%=rs.getString("email_subject")%></td></tr><%
	}
	%></table><%
...
<%
	...
	update=null;
	%><script>window.location.replace("/admin/email_message_edit.jsp?type=list");</script><%
