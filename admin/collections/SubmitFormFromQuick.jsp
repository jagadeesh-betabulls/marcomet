...
<body >
<form method="POST" name="submitForm" action="EntryFormFromQuick.jsp">
...
<%
    ...
    if(__act!=null && __act.equals("showAll")){
      response.sendRedirect("AddEntryFromQuick.jsp?showAll=y");
      return;
    }
   if(__act!=null && __act.equals("viewlist")){
    response.sendRedirect("ViewListFormFromQuick.jsp");
...
{ 
 ...
 if(__act.equals("edit")){
   response.sendRedirect("ViewListFormFromQuick.jsp?call=submit&cId="+compId.trim()+"&CURRENT_PAGE=1");
} else {
     response.sendRedirect("EntryFormFromQuick.jsp");
  }
%>
