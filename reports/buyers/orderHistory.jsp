...
if (reportType.equals("2")){
  sqlRoot = "select distinct j.root_prod_code, proot.description from orders o, projects pr, jobs j, site_hosts sh, products prod, product_roots proot where j.project_id=pr.id and pr.order_id=o.id  and o.site_host_id=sh.id  and j.product_code=prod.prod_code  and j.root_prod_code=proot.root_prod_code  and o.date_created>='" + request.getParameter("dateFrom") + "'  and o.date_created<='" + request.getParameter("dateTo") + "' and o.buyer_contact_id = "+contactId+" order by j.root_prod_code";
} else {
  sqlRoot = "select distinct 'All' as root_prod_code from jobs j";
}
ResultSet root = st2.executeQuery(sqlRoot);
if(root.next()){
  do{
    if (reportType.equals("2")){
      ...
      rTotalPrice = 0;
      rTotalTotal = 0;
      sqlOrders = "select j.root_prod_code, j.product_code, prod.prod_name,DATE_FORMAT(o.date_created,'%m/%d/%Y') 'creationDate', j.id, quantity, price from orders o, projects pr, site_hosts sh,jobs j left join products prod on j.product_id=prod.id where j.project_id=pr.id and pr.order_id=o.id  and o.site_host_id=sh.id and o.date_created>='" + request.getParameter("dateFrom") + "'  and o.date_created<='" + request.getParameter("dateTo") + "' and o.buyer_contact_id = "+contactId+" and j.root_prod_code = '"+rootProduct+"' order by o.date_created desc";
    }else{
      sqlOrders = "select j.root_prod_code, j.product_code, prod.prod_name, DATE_FORMAT(o.date_created,'%m/%d/%Y') 'creationDate', j.id, quantity, price from orders o, projects pr, site_hosts sh,jobs j left join products prod on j.product_id=prod.id where j.project_id=pr.id and pr.order_id=o.id  and o.site_host_id=sh.id and o.date_created>='" + request.getParameter("dateFrom") + "'  and o.date_created<='" + request.getParameter("dateTo") + "' and o.buyer_contact_id = "+contactId+" order by o.date_created desc";
    }
    ResultSet orders = st3.executeQuery(sqlOrders);
    if(orders.next()){
    %><%=headerRow%><%
      do{
        ...
        %>
        <tr>
        <td class="lineitemcenter<%=alt%>" ><%=orders.getString("creationDate")%></td>
...
</html><%st.close();st2.close();st3.close();conn.close();%>