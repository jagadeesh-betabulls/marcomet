...
<!----------------------Report Header------------------------------->
<div align='right' style="float:right;"><a href='javascript:history.go(-1);' class='menuLINK'>&laquo;&nbsp;Return to Filters</a></div><div class="Title"><%="Invoice History Report"%></div>
...
sqlARInvoices = "SELECT sum(acd.payment_amount) as paid, st.buyer_exempt,DATE_FORMAT(ai.creation_date,'%m/%d/%Y') 'creationDate',ai.*,arid.*,luas.value, j.job_name FROM ar_invoice_details arid, jobs j, sales_tax st, lu_abreviated_states luas,ar_invoices ai LEFT JOIN ar_collection_details acd on ai.id = acd.ar_invoiceid WHERE  ai.id = arid.ar_invoiceid AND arid.jobid = j.id AND st.entity = luas.id  AND ai.bill_to_contactid = " + contactId + "  AND st.job_id=j.id AND ai.creation_date >= '" + request.getParameter("dateFrom") +"' and ai.creation_date <= '" + request.getParameter("dateTo") + "' GROUP BY ai.id ORDER BY ai.creation_date ";
...
if(rsARInvoices.next()){
  ...
  do{
    ...
    %>
    <tr>
    <td class="lineitemcenter<%=alt%>" ><%=rsARInvoices.getString("creationDate")%></td>
