    	...
    	if(vBlocks.get(z+3).toString().equals(""+i)){
	    	if(vBlocks.get(z+4).toString().equals("800")){//if this is an image field
	    		if(vBlocks.get(z+5)!=null && !vBlocks.get(z+5).toString().equals("")){ //if there is an image mask specified
	    			...
	    			int mask2 = pp.load_image("auto",searchpath+"/"+vBlocks.get(z+5).toString(), "mask"); 
					if (mask == -1)
					...
					int impImage2 = pp.load_image("auto", ((vBlocks.get(z+1).toString().indexOf("transfers")>0)?homeDir:tempPath) +vBlocks.get(z+1).toString(), optlist);
					if (impImage == -1) 
						throw new Exception("Error: " + p.get_errmsg()); 
					if(p.fill_imageblock(pdipage, vBlocks.get(z).toString(), impImage,"")==-1)
	    	    		System.err.println("Warning: " + p.get_errmsg());
	    	    	if(pp.fill_imageblock(p_pdipage, vBlocks.get(z).toString(), impImage2,"")==-1)
	    	    		System.err.println("Warning: " + pp.get_errmsg());
	    		    ...
	    		    int impImage2=pp.load_image("auto", ((vBlocks.get(z+1).toString().indexOf("transfers")>0)?homeDir:tempPath)+vBlocks.get(z+1).toString(),""); 
					if(pp.fill_imageblock(p_pdipage, vBlocks.get(z).toString(), impImage2,"")==-1)
	    		    	System.err.println("Image Block Warning: " + pp.get_errmsg());
	    		}
	    		x++;
...
/*    
	...
	if (decoder.isFileViewable()){	
	   ...
	   */
    Process p1=Runtime.getRuntime().exec("convert "+tempPath+pdfFileName+" "+tempPath + imageFileName);
    p1.waitFor();
