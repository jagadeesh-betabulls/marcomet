<%@page import="java.util.*,javax.servlet.*, com.marcomet.catalog.*, com.pdflib.*,org.jpedal.*,java.io.*,javax.imageio.*,java.awt.Graphics,java.awt.Image,java.awt.image.*,com.marcomet.catalog.*,com.marcomet.jdbc.*,java.sql.*,org.apache.commons.lang.*" %><%
	    	...
	    	if(vBlocks.get(z+4).toString().equals("800")){//if this is an image field
				...
				if (pdipage2 == -1) {
			    	...
			    	if(lastBlockName.equals(vBlocks.get(z).toString())){
			    		fillText=lastFillText+vBlocks.get(z+7).toString()+  vBlocks.get(z+8).toString()+fillText;
			    	}

					if(vBlocks.get(z+6).toString().equals("0") || vBlocks.get(z+6).toString().equals("2")){
						if (p.fill_textblock(pdipage, vBlocks.get(z).toString(),  vBlocks.get(z+8).toString()+fillText,"embedding=true encoding=unicode")==-1) {
