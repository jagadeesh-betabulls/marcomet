...
function sql_query2($param, $fetchrows="") {
	return $param;
}

if (!function_exists("do_search")) {
function do_search($search,$restypes="",$order_by="relevance",$archive=0,$fetchrows=-1, $brand_type="", $initiative_type="", $filtersql=array(), $variants="")
	{
	...
	$orig_order=$order_by;
	$order=array("relevance"=>"score desc, creation_date desc","popularity"=>"hit_count desc, creation_date desc","rating"=>"rating desc, score desc","date"=>"creation_date desc","colour"=>"has_image desc,image_blue,image_green,image_red,creation_date","country"=>"country", "file_extension"=>"has_image desc,file_extension", "name" => "title");
	...
	# append brand filtering
	if($brand_type!="") {
		if ($filter!="") {$filter.=" and ";}
		$filter.="ref IN (SELECT resource FROM resource_data WHERE resource_type_field=63 AND value='$brand_type')";
	}
	
	# append initiative filtering
	if($initiative_type!="") {
		if ($filter!="") {$filter.=" and ";}
		$filter.="ref IN (SELECT resource FROM resource_data WHERE resource_type_field=69 AND value='$initiative_type')";
	}
	
	// Add additional filters
	for($i=0;$i<count($filtersql);$i++) {
		if ($filter!="") {$filter.=" and ";}
		$filter.=$filtersql[$i];
	}
	...
	// Replace all occurrences of $username with the current username in the search filter
	global $userref; // Get the userref
	if($userref) {
		$username =  sql_query("SELECT * from user WHERE ref=$userref"); // Get the username
		$usersearchfilter = str_replace("\$username", $username[0]["username"], $usersearchfilter);
	}
	
	if($variants=="yes") {
		$usersearchfilter = str_replace("is_child_variant=no|", "", $usersearchfilter);
		$usersearchfilter = str_replace("is_child_variant=no", "", $usersearchfilter);
	} elseif($variants=="no") {
		if(strpos($usersearchfilter, "is_child_variant")===false) {
			$usersearchfilter .= "is_child_variant=no|";
		}
	}
	
	
	// Should we not allow resourcetype 8, 7, and 6
	$usersearchfilter2 = str_replace("\$restype12;", "", $usersearchfilter);
	if($usersearchfilter2!=$usersearchfilter) {
		$usersearchfilter = $usersearchfilter2;
		//$restypes = "";
	}
	...
	// CHANGE: 6/24/08 - The following was commented out to allow a search for numeric keywords
	/*
...
if (!function_exists("do_search_sql")) {
function do_search_sql($search,$restypes="",$order_by="relevance",$archive=0,$fetchrows=-1, $brand_type="", $initiative_type="", $filtersql=array())
	{
	# Takes a search string $search, as provided by the user, and returns a results set
	# of matching resources.
	# If there are no matches, instead returns an array of suggested searches.
	...
	# resolve $order_by to something meaningful in sql
	$orig_order=$order_by;
	$order=array("relevance"=>"score desc, creation_date desc","popularity"=>"hit_count desc, creation_date desc","rating"=>"rating desc, score desc","date"=>"creation_date desc","colour"=>"has_image desc,image_blue,image_green,image_red,creation_date","country"=>"country", "file_extension"=>"has_image desc,file_extension");
	$order_by=$order[$order_by];
	$keywords=split_keywords($search);
	$search=trim($search); # remove any trailing or leading spaces
	...
	if($userref) {
		$username =  sql_query("SELECT * from user WHERE ref=$userref"); // Get the username
		$usersearchfilter = str_replace("\$username", str_pad($username[0]["username"], 5, "00000", STR_PAD_LEFT), $usersearchfilter);
	}
	...
	if (substr($search,0,5)=="!last") 
		...
		return sql_query2("select *,hit_count score from (select r.* from resource r $custperm where $filter order by ref desc limit " . str_replace("!last","",$search) . ") r2 order by $order_by",false,$fetchrows);
		}
	
	# Duplicate Resources (based on file_checksum)
	if (substr($search,0,11)=="!duplicates") 
		{
		return sql_query2("select *,hit_count score from resource r $custperm where $filter and file_checksum in (select file_checksum from (select file_checksum,count(*) dupecount from resource group by file_checksum) r2 where r2.dupecount>1) order by file_checksum",false,$fetchrows);
		}
	...
	if (substr($search,0,11)=="!collection")
		...
		if (getval("k","")!="") {$colcustperm="";$filter="ref>0";} # Special case if a key has been provided.
		#echo "<!--select r.*,r.hit_count score from resource r join collection_resource c on r.ref=c.resource $colcustperm where c.collection='" . str_replace("!collection","",$search) . "' and $filter group by r.ref order by $order_by;-->";
		return sql_query2("select r.*,c.*,r.hit_count score,length(c.comment) commentset from resource r join collection_resource c on r.ref=c.resource $colcustperm where c.collection='" . str_replace("!collection","",$search) . "' and $filter group by r.ref order by $order_by;",false,$fetchrows);
		}
	
	# View Related
	if (substr($search,0,8)=="!related") return sql_query2("select r.*,r.hit_count score from resource r join resource_related t on ((t.related=r.ref and t.resource='" . str_replace("!related","",$search) . "') or (t.resource=r.ref and t.related='" . str_replace("!related","",$search) . "')) $custperm where 1=1 and $filter group by r.ref order by $order_by;",false,$fetchrows);
	
	# Similar to a colour
	if (substr($search,0,4)=="!rgb")
		{
		$rgb=explode(":",$search);$rgb=explode(",",$rgb[1]);
		return sql_query2("select r.* from resource r $custperm where has_image=1 and $filter group by r.ref order by (abs(image_red-" . $rgb[0] . ")+abs(image_green-" . $rgb[1] . ")+abs(image_blue-" . $rgb[2] . ")) asc limit 500;",false,$fetchrows);
		}
		
	# Similar to a colour by key
	if (substr($search,0,10)=="!colourkey")
		{
		return sql_query2("select r.* from resource r $custperm where has_image=1 and left(colour_key,4)='" . substr(str_replace("!colourkey","",$search),0,4) . "' and $filter group by r.ref",false,$fetchrows);
		}
	...
	if (is_numeric($search)) 
		{
		return sql_query2("select r.*,hit_count score from resource r $custperm where ref='$search' and $filter group by r.ref");
		}*/
	
	# Searching for pending archive
	if ($search=="!archivepending")
		{
		return sql_query2("select r.*,hit_count score from resource r $custperm where archive=1 and ref>0 group by r.ref order by $order_by",false,$fetchrows);
		}
	...
	if (substr($search,0,14)=="!contributions") 
		{
		return sql_query2("select r.*,hit_count score from resource r $custperm where created_by='" . str_replace("!contributions","",$search) . "' and $filter group by r.ref order by $order_by",false,$fetchrows);
		}
	
	# Search for resources with images
	if ($search=="!images") return sql_query2("select r.*,hit_count score from resource r $custperm where has_image=1 group by r.ref order by $order_by",false,$fetchrows);
	...
	if ($fullmatch==false)
		...
		if ($suggested==$keywords)
			...
			return array("NOTFOUND_SUGGEST", $suggest);
			}
		}
	...
	if (($t2!="") && ($sql!="")) {$sql=" and " . $sql;}
	$sql="select r.*,$score score from resource r" . $t . " where $t2 $sql group by r.ref order by $order_by limit $max_results";
	$result=sql_query2($sql,false,$fetchrows);#echo $sql;
	if (count($result)>0) {return $result;}
