...
#example get_resource_path($ref,"thm",false,$result[$n]["preview_extension"])
function get_resource_path($ref,$size,$generate,$extension="jpg",$scramble=-1,$amsDomain="wam.marcomet.com")
	{
	...
	$folder="http://$amsDomain/DigitalAssets/filestore/";
...
function get_resource_types()
	...
	if($usersearchfilter2!=$usersearchfilter) {
		$type_filter = " WHERE ref IN (1,2,3,4,5)";
	}
	
	
	
	# Returns a list of resource types.
	$r=sql_query("select * from resource_type$type_filter order by ref");
	# Translate names
	for ($n=0;$n<count($r);$n++)
		{
...
// Modified to support numeric keywords
/*
length(k.keyword)>=3 and length(k.keyword)<=15 and k.keyword not like '%0%' and k.keyword not like '%1%' and k.keyword not like '%2%' and k.keyword not like '%3%' and k.keyword not like '%4%' and k.keyword not like '%5%' and k.keyword not like '%6%' and k.keyword not like '%7%' and k.keyword not like '%8%' and k.keyword not like '%9%'
*/
function get_resource_top_keywords($resource,$count)
	...
	return sql_array("select distinct k.ref,k.keyword value from keyword k,resource_keyword r,resource_type_field f where k.ref=r.keyword and r.resource='$resource' and f.ref=r.resource_type_field and f.use_for_similar=1 order by k.hit_count desc limit $count");
	}
