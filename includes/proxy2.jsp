...
String sql1 = "SELECT id, firstname,lastname, email, companyid, md5(id) 'cookie encryption' FROM contacts WHERE id = ?";	
	...
	if(rs1.next()){
		...
		request.getSession().setAttribute("homeNotice","<form id='proxyArea'>Proxy Order for "+rs1.getString("firstname")+" " + rs1.getString("lastname")+", Proxied by Customer Service Rep ID#"+((session.getAttribute("proxyId")==null)?"NONE":session.getAttribute("proxyId"))+". Please logout and close window when finished.<br><a href='/servlet/com.marcomet.catalog.CatalogNavigationServlet?offeringId=1009&jobTypeId=9998&serviceTypeId=9999&productId=5324' target='"+target+"'>&raquo;Click here to place a custom order for this client.</a><br>To directly order a product enter the product code here: <input type='text' size='20' name='prodCode'><a href='javascript:orderProd()' class='greybutton'>Order</a>"+((rs1.getString("email").equals(""))?"<br><font color=red>NOTE: No Email on file for this user, please go to User Info and review all contact information with the customer.</font>":"")+"<script>function orderProd(){if(document.forms[0].prodCode.value!=''){frames['"+target+"'].window.location='/contents/directOrderProduct.jsp?prodCode='+document.forms[0].prodCode.value;}else{alert(\"Please enter a product code before ordering.\");}}</script></form>");
