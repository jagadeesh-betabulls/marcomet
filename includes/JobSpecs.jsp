...
ResultSet rs1 = st.executeQuery("SELECT ls.value AS label, js.value AS value,js.id as id FROM job_specs js, lu_specs ls, catalog_specs cs WHERE ls.id != 88888 AND ls.id != 99999 AND ls.id = cs.spec_id AND cs.id = js.cat_spec_id AND js.job_id = " + jobId + " ORDER BY ls.sequence");
			...
			while (rs1.next()){  
	   			...
	   			<td class="body"><%
	   			if (editor){
	   				%><a href='javascript:pop("/popups/QuickChangeForm.jsp?cols=60&rows=3&question=Change%20<%=rs1.getString("label")%>&primaryKeyValue=<%=rs1.getString("id")%>&columnName=value&tableName=job_specs&valueType=string",500,200)' class='minderLink'>&raquo;&nbsp;<%=rs1.getString("value")%></a><%	   			}else{
