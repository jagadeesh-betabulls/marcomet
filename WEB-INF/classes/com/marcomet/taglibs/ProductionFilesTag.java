...
public class ProductionFilesTag extends TagSupport {
	...
	public final  int doEndTag() throws JspException {
		...
		try {
			...
			production = this.buildWorkingHeader("Production Files:");

			String query = "select fmd.id, file_name, fmd.company_id as companyid, file_size, description, status, DATE_FORMAT(creation_date,'%m/%d/%y') as creation_date, firstname, lastname, job_id, project_id, group_id from file_meta_data fmd, contacts c where c.id = fmd.user_id and category = 'Final' and job_id = " + jobId + " order by creation_date desc";
