...
<%
   ...
   if (countRS.next() && countRS.getInt("count") > 0) {
      String productionQuery = "SELECT fmd.id, file_name, fmd.company_id AS company_id, file_size, description, status, DATE_FORMAT(creation_date,'%m/%d/%y') as creation_date, firstname AS first_name, lastname AS last_name, job_id, project_id, group_id FROM file_meta_data fmd, contacts c WHERE c.id = fmd.user_id AND category = 'Final' AND job_id = " + request.getParameter("jobId") + " ORDER BY creation_date DESC";
