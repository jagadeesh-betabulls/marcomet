...
String status=((!submitted || request.getParameter("status")==null)?((selfDesigned)?"":"Final"):request.getParameter("status"));
String category=((!submitted || request.getParameter("category")==null)?((selfDesigned)?"":"Final"):request.getParameter("category"));
...
function downloadFile(fileName){
	dlFrame = document.getElementById("downloadFile");
	dlFrame.innerHTML = '<iframe src="http://<%=baseURL%>/popups/downloadOneFile.jsp?modal=true&dlFile='+fileName+'" width="100%" height="100%" frameborder="0" marginheight="0" marginwidth="0" scrolling="auto" id="popupImgIframe"><p>Please upgrade your web browser.</p></iframe>';
}


</script>
</head>
<body><div id="downloadFile" style="display:none;"></div>
<form method="post" action=""><input type="hidden" name="submitted" value="true"><%
%><table border="0" cellpadding="3" cellspacing="0" width="100%">
<tr><td valign="bottom" width="10%"><div class="subtitle">File&nbsp;Manager<br><%=((selfDesigned)?"DIY Designed":"Uploaded")%>&nbsp;Files </div>
<%
//BEGIN FILE FILTERS 
...
%>
    <tr> 
      <td colspan="2"><hr size=1 color=gray>
        <p class="body"><%if (selfDesigned){%>All files which have been designed by you in the Do-It-Yourself Design Center are included below. <%}else{%>All files associated to your jobs are included below categorized as Final Files, Working Files and Files for Approval.<%}%> Many of the file types (.jpg, .pdf, etc.) may be viewed online by clicking on their filename.</p>
      </td></tr>
      </table>
</form>
    <%
  ...
  if (((RoleResolver)session.getAttribute("roles")).isVendor()) {
     query = "SELECT fmd.id, file_name, fmd.company_id AS companyid, file_size, fmd.description, reply, category, status, DATE_FORMAT(fmd.creation_date,'%m/%d/%y') AS creation_date, firstname, lastname, job_id, j.project_id,j.job_name FROM file_meta_data fmd, contacts c, jobs j, vendors v WHERE "+((selfDesigned)?"source=2 AND ":"")+" c.id = fmd.user_id AND fmd.job_id = j.id AND j.vendor_id = v.id AND v.company_id = " + companyId+dateFilter+((jobId.equals(""))?"":" AND j.id="+jobId)+((category.equals(""))?"":" AND fmd.category='"+category+"'")+((status.equals(""))?"":" AND fmd.status='"+status+"'")+" order by fmd.creation_date desc";
  } else {
     query = "SELECT fmd.id, file_name, fmd.company_id AS companyid, file_size, fmd.description, reply, category, status, DATE_FORMAT(fmd.creation_date,'%m/%d/%y') AS creation_date, job_id, j.project_id, firstname, lastname,j.job_name FROM file_meta_data fmd, contacts c, jobs j, projects p, orders o, products prod WHERE "+((selfDesigned)?"fmd.category<>'For Appvl' AND fmd.file_size<>0 AND prod.allow_printfile=1 ":"fmd.category<>'For Appvl' AND fmd.file_size<>0 AND prod.allow_printfile=0 ")+" and user_id = c.id AND j.id = fmd.job_id AND fmd.project_id = p.id AND o.id = p.order_id AND j.product_id = prod.id AND (j.jbuyer_contact_id = "+contactId+" or j.jbuyer_company_id = " + companyId+") "+dateFilter+((jobId.equals(""))?"":" AND j.id="+jobId) +((category.equals(""))?"":" AND fmd.category='"+category+"'")+((status.equals(""))?"":" AND fmd.status='"+status+"'")+" order by fmd.creation_date desc";
  }
...
%>
    <tr> 
      <td class=lineitem<%=(alt)?"alt":""%> align="left" width="3%"> 
        <input type="checkbox" name="fileList" 	value="<%= rs.getString("id") %>">
        <script>files[fileCount++]='/transfers/<%= rs.getString("companyid") %>/<%= rs.getString("file_name") %>'</script>
      </td><%
      if(selfDesigned){
      %><td class=lineitem<%=(alt)?"alt":""%> align="left" width="5%"> 
        <a href="/catalog/reprintJobsFromFile.jsp?jobId=<%= rs.getString("job_id") %>" class='minderLink'>&raquo;&nbsp;Print&nbsp;This&nbsp;File&nbsp;for&nbsp;Me</a><br><a href="/catalog/reprintJobsFromFile.jsp?jobId=<%= rs.getString("job_id") %>&gtc=true" class="minderLink">&raquo;&nbsp;Edit &nbsp;New&nbsp;Version</a>
    </td><%}%>
      <td class=lineitem<%=(alt)?"alt":""%> ><a href="javascript:downloadFile('/transfers/<%= rs.getString("companyid") %>/<%= rs.getString("file_name") %>');" class="minderLink" ><%= rs.getString("file_name") %></a></td>
