...
<%
    ...
    String emailrf = request.getParameter("emailrf");
...
<html>
  ...
  <body onLoad="MM_preloadImages('/images/buttons/checkover.gif','/iamges/buttons/cancelover.gif')" background="<%=(String) session.getAttribute("siteHostRoot")%>/images/back3.jpg">
      ...
      <table border="0" width="600">
        <tr>
          ...
          <td class="body">
            <%
    if (jobyn) {
      if (emailrf != null && emailrf.equals("forward")) {
            %>
            <input type="text" name="to" value="">&nbsp;&nbsp;Separate multiple emails with a comma (,)&nbsp;&nbsp;
            <%              } else {
            ...
            <%
              ...
              if (rs1.next() && !rs1.getString("email").equals("")) {
            %>
            "<%=rs1.getString("firstname") + " " + rs1.getString("lastname")%>" <%= "&lt;" + rs1.getString("email") + "&gt;"%>
            <input type="hidden" name="to" value="<%= rs1.getString("email")%>">
            <%
              } else {
            %>
    ...
    if (session.getAttribute("contactId") != null) {
      ...
      if (rs1.next()) {
            %>
            "<%=(String) session.getAttribute("UserFullName")%>" <%= "&lt;" + rs1.getString("email") + "&gt;"%>
            <input type="hidden" name="from" value="<%= rs1.getString("email")%>">
            <%
              }
            } else {%>
            <input type="text" name="from" value="">&nbsp;&nbsp;Separate multiple emails with a comma (,)&nbsp;&nbsp;
            <%    }%>
          </td>
        </tr>
        <tr>
...
%>
      ...
      <table>
        ...
        <tr>
          <td class="body">
            <input name="attachmentOption" value="1" type="radio" checked>&nbsp;Send Link to Attachments
            <input name="attachmentOption" value="2" type="radio">&nbsp;Send Attachments in Email
          </td>
        </tr>
        <% 	String[] fileIds = null;
    int length = 0;
    ...
    try {
      ...
      <%
    if (jobyn) {
      sql = "SELECT esh.* FROM email_sent_histories esh LEFT JOIN contacts c_from ON esh.email_from_id=c_from.id LEFT JOIN contacts c_to ON esh.email_to_id=c_to.id WHERE esh.sent<>'0' AND esh.job_id=" + request.getParameter("jobId");
      rs1 = st3.executeQuery(sql);
      if (rs1.next()) {
      %>
      <input type="hidden" name="emailId" value="<%= rs1.getString("id")%>">
      <input type="hidden" name="emailToId" value="<%= rs1.getString("email_to_id")%>">
      <input type="hidden" name="emailFromId" value="<%= rs1.getString("email_from_id")%>">
      <input type="hidden" name="subjectRE" value="<%= rs1.getString("subject")%>">
      <%}
    }
    if (emailrf != null) {
      %>
      <input type="hidden" name="emailRF" value="<%= emailrf%>">
      <%}
      %>
